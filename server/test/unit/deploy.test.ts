...
const cmdOf = (argv: string[]) => argv.find((a) => ["config", "build", "up", "ps", "down", "logs"].includes(a))!;
...
function setup(script: Script = {}) {
  ...
  const calls: { cmd: string; argv: string[]; cwd: string; routesAtCall: number; stack: string | null }[] = [];
  const record = (argv: string[], cwd: string) => {
    const f = argv[argv.indexOf("--file") + 1];
    calls.push({ cmd: cmdOf(argv), argv, cwd, routesAtCall: table.size, stack: f && existsSync(f) ? readFileSync(f, "utf8") : null });
  };
  ...
  const compose: ComposeRunner = {
    async *stream(argv, _host, o): AsyncGenerator<ComposeEvent> {
      ...
      if (up.hang) await new Promise<void>((resolve) => o.signal?.addEventListener("abort", () => resolve()));
    ...
    async capture(argv, _host, o): Promise<ComposeResult> {
      ...
      switch (cmdOf(argv)) {
        case "config": {
          ...
          const doc = script.config?.(file) ?? { services: JSON.parse(readFileSync(file, "utf8")).services, networks: { default: { name: "gw-plan_default" } } };
          if (doc === "invalid") return { code: 15, stdout: "", stderr: "yaml: line 3: did not find expected key", signal: null };
          return ok(JSON.stringify(doc));
        }
        case "ps": return ok((psQueue.length > 1 ? psQueue.shift()! : psQueue[0]!).map((r) => JSON.stringify(r)).join("\n"));
        case "logs": return ok("web-1  | Error: listen EADDRINUSE");
        case "down":
          if (script.down === "throw") throw new Error("connect ECONNREFUSED");
          return { code: script.down?.code ?? 0, stdout: "", stderr: script.down?.stderr ?? "", signal: null };
      }
      return ok("");
    },
  };
  ...
  const events = () => bus && new EventsRepo(db).since(0).map((e) => `${e.type}${e.payload["state"] ? `:${String(e.payload["state"])}` : ""}`);
...
describe("deploy: the happy path", () => {
  ...
  test("§5: the route row exists BEFORE `compose up` runs -- never after", async () => {
    ...
    await (await deploy(ctx, input())).done;
  ...
  test("waits out `starting` health and a port that is not answering yet", async () => {
    const { ctx, input, logs } = setup({
      ps: [[{ Service: "web", State: "running", Health: "starting" }], [{ Service: "web", State: "running", Health: "healthy" }]],
  ...
  test("a one-shot sidecar that exited 0 does not hold the stack back", async () => {
    const { ctx, input } = setup({ ps: [[{ Service: "web", State: "running" }, { Service: "migrate", State: "exited", ExitCode: 0 }]] });
...
describe("deploy: planning failures leave nothing behind and are the caller's 4xx", () => {
  const nothingLeft = (s: ReturnType<typeof setup>) => {
    ...
    expect(existsSync(join(s.dir, "work")) ? Bun.spawnSync(["ls", join(s.dir, "work")]).stdout.toString() : "").toBe("");
  };

  test("an invalid compose file", async () => {
    const s = setup({ config: () => "invalid" });
    await expect(deploy(s.ctx, s.input())).rejects.toMatchObject({ status: 422, detail: { compose: expect.stringContaining("did not find expected key") } });
    nothingLeft(s);
  });

  test("a policy violation", async () => {
    const s = setup({ config: () => ({ services: { web: { image: "x", privileged: true, ports: [{ target: 80, protocol: "tcp" }] } } }) });
    await expect(deploy(s.ctx, s.input())).rejects.toMatchObject({ status: 422, detail: { violations: ['service "web": privileged is not allowed'] } });
    nothingLeft(s);
  });
  ...
  test("a live name is a 409; a destroyed one is reusable", async () => {
    ...
    await first.done;
    await expect(deploy(s.ctx, s.input())).rejects.toMatchObject({ status: 409, detail: { previewId: first.preview.id } });
    ...
    expect(s.previews.list({ includeDestroyed: true }).map((p) => p.id)).toEqual([second.preview.id]);
  });
  ...
  test("concurrent deploys never share a port", async () => {
    const s = setup();
    const all = await Promise.all(["c1", "c2", "c3"].map((name) => deploy(s.ctx, s.input({ name }))));
...
describe("deploy: run failures leave a `failed` preview that explains itself", () => {
  test("`up` exits non-zero: container logs are salvaged, the stack is torn down, the route stays", async () => {
    const s = setup({ up: { code: 1, lines: ["Error response from daemon: port is already allocated"] } });
  ...
  test("a crashed service, an unhealthy one, and a start timeout", async () => {
    ...
    expect((await (await deploy(crashed.ctx, crashed.input())).done).error).toBe('service "web" exited with code 137');
    const sick = setup({ ps: [[{ Service: "web", State: "running", Health: "unhealthy" }]] });
    expect((await (await deploy(sick.ctx, sick.input())).done).error).toBe('service "web" is unhealthy');
    const slow = setup({ ps: [[{ Service: "web", State: "running", Health: "starting" }]] });
    expect((await (await deploy(slow.ctx, slow.input())).done).error).toMatch(/timed out after .* waiting for web: starting/);
  });

  test("healthy but never answering HTTP names the likely causes", async () => {
    const s = setup({ probe: [false] });
    expect((await (await deploy(s.ctx, s.input())).done).error).toMatch(/web:3000 never answered HTTP.*0\.0\.0\.0/);
  });
});
...
describe("destroy", () => {
  test("file-less down from an empty directory, then routes, state, logs", async () => {
    ...
    expect(s.calls.slice(-2).map((c) => c.argv)).toEqual([
      ["docker", "volume", "ls", "--quiet", "--filter", "label=com.docker.compose.project=gw-default-web-app"],
      ["docker", "image", "ls", "--quiet", "--filter", "dangling=true", "--filter", "label=com.docker.compose.project=gw-default-web-app"],
    ]);
  ...
  test("if the daemon cannot confirm, NOTHING is released: routes and ports stay claimed", async () => {
    for (const down of [{ code: 1, stderr: "cannot connect" }, "throw"] as const) {
      ...
      expect(s.previews.get(res.preview.id)).toMatchObject({ state: "failed", error: expect.stringContaining("destroy failed") });
