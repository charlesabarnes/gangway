...
function make(actor: Actor = ACTOR, maxQueue?: number) {
  ...
  previewRoutes(api, s.ctx, null as never, { heartbeatMs: 10_000, ...(maxQueue === undefined ? {} : { maxQueue }) });
...
async function frames(res: Response, count: number, timeoutMs = 3000) {
  ...
  while (out.length < count && Date.now() < deadline) {
    ...
    while ((i = buf.indexOf("\n\n")) >= 0) {
      ...
      buf = buf.slice(i + 2);
      if (f["event"] === "log") { const d = JSON.parse(f["data"]!) as { stream: string; line: string }; out.push({ id: Number(f["id"]), ...d }); }
    }
  }
...
describe("GET /v1/previews", () => {
  test("carries the event cursor to follow from, read before the list", async () => {
    const t = make();
    const before = (await (await t.get("/previews")).json() as { seq: number; previews: unknown[] });
    expect(before).toEqual({ seq: 0, previews: [] });
    const p = await t.deployed("one");
    const after = await (await t.get("/previews")).json() as { seq: number; previews: { id: string; urls: unknown[] }[] };
  ...
  test("state is repeatable or comma-joined; destroyed is left out unless asked for", async () => {
    ...
    const ids = async (q: string) => ((await (await t.get(`/previews${q}`)).json()) as { previews: { id: string }[] }).previews.map((p) => p.id).sort();
...
describe("GET /v1/previews/:id/events and /builds", () => {
  ...
  test("builds: none for an image deploy; an unknown or malformed id is 404 before it names a file", async () => {
    ...
    for (const path of ["/previews/nope/events", "/previews/..%2F..%2Fetc/builds", "/previews/01ARZ3NDEKTSV4RRFFQ69G5FAV/events"]) {
  ...
  test("a read-scoped token may read all of it; none of it without the permission", async () => {
    ...
    for (const path of ["/previews", `/previews/${p.id}`, `/previews/${p.id}/events`, `/previews/${p.id}/builds`]) expect((await reader.get(path)).status).toBe(200);
    expect((await reader.get(`/previews/${p.id}`, { method: "DELETE" })).status).toBe(403);

    const nobody = make({ kind: "user", userId: "u", roleId: "empty", permissions: new Set(), sessionId: "s" });
    for (const path of ["/previews", "/previews/x/events", "/previews/x/builds", "/previews/x/logs"]) expect((await nobody.get(path)).status).toBe(403);
  });
});
...
describe("GET /v1/previews/:id/logs", () => {
  const longLog = async (t: ReturnType<typeof make>, lines: number) => {
    ...
    t.ctx.logs.append(p.id, "build", Array.from({ length: lines - already }, (_, i) => `step ${already + i + 1}`).join("\n"));
  ...
  test("REGRESSION: a log longer than the SSE queue still opens -- it used to close before its first frame, forever", async () => {
    ...
    expect(out[0]).toMatchObject({ id: 1200 - 498, stream: "system", line: "... 702 earlier lines not shown" });
  ...
  test("?tail=N starts from the last N and says what it skipped; a short log is not annotated", async () => {
    ...
    expect(tailed.out.map((f) => f.id)).toEqual([290, 291, 292, 293, 294, 295, 296, 297, 298, 299, 300]);
    expect(tailed.out[0]).toMatchObject({ stream: "system", line: "... 290 earlier lines not shown" });
    ...
    for (const bad of ["0", "-5", "many", "5001"]) expect((await t.get(`/previews/${p.id}/logs?tail=${bad}`)).status).toBe(422);
  });

  test("resuming with Last-Event-ID ignores tail's cut: exactly the lines after the cursor, no notice", async () => {
    const t = make();
    const p = await longLog(t, 300);
    const resumed = await frames(await t.get(`/previews/${p.id}/logs?tail=2000`, { headers: { "last-event-id": "295" } }), 5);
  ...
  test("lines appended while the stream is open still arrive, after the replay, once", async () => {
    ...
    expect(out.map((f) => f.line)).toEqual(["... 47 earlier lines not shown", "step 48", "step 49", "step 50", "live one", "live two"]);
  });
});
