...
import {
  NEUTRALISED_ENV, buildArgv, composeArgv, composeCapture, composeEnv, downArgv,
  parseComposePs, psArgv, runArgv, runCompose, upArgv, type ComposeEvent, type Spawner,
...
describe("composeArgv", () => {
  test("the shape of a basic invocation", () => {
    expect(composeArgv({ ...base, command: "up", args: ["-d"] })).toEqual([
      "docker", "compose",
      "--project-name", "gw-acme-pr-123",
      "--file", "/srv/gw/acme-pr-123/compose.yaml",
      "up", "-d",
    ]);
  });
  ...
  test("every global flag precedes the subcommand", () => {
    ...
    for (const flag of ["--project-name", "--project-directory", "--file", "--env-file", "--profile"]) {
  ...
  test("the binary is overridable but defaults to docker", () => {
    ...
    expect(composeArgv({ ...base, command: "ps", docker: "/usr/local/bin/docker" })[0])
...
describe("the five commands we actually run", () => {
  ...
  test("build uses plain progress, the only parseable mode, with services last", () => {
    expect(buildArgv(base, ["api", "web"]).slice(-4))
      .toEqual(["build", "--progress=plain", "api", "web"]);
  });
  ...
  test("run is --rm, with the service before its command", () => {
    expect(runArgv(base, "api", ["./scripts/seed.sh"]).slice(-4))
      .toEqual(["run", "--rm", "api", "./scripts/seed.sh"]);
    expect(runArgv(base, "api", ["sh", "-c", "echo hi"], ["-e", "SEED=1"]).slice(-8))
      .toEqual(["run", "--rm", "-e", "SEED=1", "api", "sh", "-c", "echo hi"]);
  });
});
...
function fakeSpawner(o: FakeOpts): Spawner {
  const empty = async function* (): AsyncGenerator<string> { /* nothing */ };
  return (argv, opts) => {
    o.onSpawn?.(argv, opts.env, opts.cwd);
    let done: () => void = () => {};
    const exited = new Promise<number>((resolve) => { done = () => resolve(o.code ?? 0); });
...
describe("runCompose", () => {
  test("yields lines then a single exit event carrying the code", async () => {
    ...
    const spawner = fakeSpawner({
      stdout: async function* () { yield " Container gw-api-1  Started\n"; },
  ...
  test("stdout and stderr interleave rather than draining one at a time", async () => {
    let step1: () => void = () => {};
    let step2: () => void = () => {};
    const a = new Promise<void>((r) => { step1 = r; });
    const b = new Promise<void>((r) => { step2 = r; });
    const spawner = fakeSpawner({
      stdout: async function* () { yield "out-1\n"; await b; yield "out-2\n"; },
      stderr: async function* () { await a; yield "err-1\n"; step2(); },
    });
  ...
  test("chunk boundaries do not split lines, and a trailing partial line is emitted", async () => {
    const spawner = fakeSpawner({
      stdout: async function* () { yield "hel"; yield "lo\r\nwor"; yield "ld\nno-newline"; },
    });
    const lines: string[] = [];
  ...
  test("a non-zero exit is reported, not thrown", async () => {
    const spawner = fakeSpawner({ stderr: async function* () { yield "no such service\n"; }, code: 1 });
    const events: ComposeEvent[] = [];
  ...
  test("composeCapture buffers for value-shaped output", async () => {
    const spawner = fakeSpawner({
      stdout: async function* () { yield '{"Name":"gw-api-1"}\n'; },
      stderr: async function* () { yield "warn\n"; },
      code: 0,
    });
...
describe("parseComposePs", () => {
  const row = {
    Name: "gw-acme-pr-123-api-1", Service: "api", State: "running", Health: "healthy",
    ExitCode: 0,
