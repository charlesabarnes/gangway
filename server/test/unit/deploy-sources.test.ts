...
describe("repository secrets become .env (ADR-0012)", () => {
  ...
  test("absent on the input, the context supplies the secrets at the template's clearance (ADR-0013)", async () => {
    ...
    s.ctx.secretsFor = (repoId, clearance) => { asked.push([repoId, clearance]); return { FROM_CTX: "1" }; };
    const archive = await tarball([{ name: "compose.yaml", content: COMPOSE }, { name: "Dockerfile", content: "FROM nginx" }]);
  ...
  test("a template at clearance none writes no .env and never asks for secrets", async () => {
    ...
    s.ctx.secretsFor = () => { throw new Error("must not be asked"); };
    const archive = await tarball([{ name: "compose.yaml", content: COMPOSE }, { name: "Dockerfile", content: "FROM nginx" }]);
    const res = await deploy(s.ctx, { ...base, name: "bare", source: { kind: "tarball", archive } });
    await res.done;
    expect(read()).toBeNull();
