...
const port = (target: number, published = String(target), protocol = "tcp") => ({
  mode: "ingress",
  target,
  published,
  protocol,
...
const plan = (m: ComposeModel, slug = "acme") =>
  planRoutes({
    ...
    const m = model(
      {
        api: {
          image: "nginx",
          ports: [port(80, "3000"), port(9000, "9000", "udp")],
...
`),
  ...
  test("x-gangway is strict: a typo is an error, not a silently ignored key", () => {
    expect(() => model({ api: { image: "n", "x-gangway": { exposed: true } } })).toThrow(
      /service "api": x-gangway/,
    );
    expect(() => model({ api: { image: "n" } }, { "x-gangway": { ttl: "forever" } })).toThrow(
      /x-gangway\.ttl/,
    );
  ...
  test("policy: every way out of the project namespace is named", () => {
    const m = model(
      ...
      {
        networks: {
          default: { name: "gw-x_default" },
          lan: { name: "br0", external: true },
          mac: { name: "gw-x_mac", driver: "macvlan" },
        },
        volumes: {
          data: { name: "gw-x_data" },
          host: { name: "gw-x_host", driver_opts: { type: "none", o: "bind", device: "/" } },
          shared: { name: "appdata" },
        },
      },
    );
...
describe("selectExposed", () => {
  test("default: the single service that publishes a port", () => {
    expect(
      selectExposed(model({ db: { image: "pg" }, web: { image: "n", ports: [port(3000)] } })),
  ...
  test("ambiguity is an error that names the fix", () => {
    ...
    expect(() =>
      selectExposed(
        model({ a: { image: "n", ports: [port(1)] }, b: { image: "n", ports: [port(2)] } }),
      ),
    ).toThrow(/ambiguous: a, b/);
    expect(() => selectExposed(model({ a: { image: "n", ports: [port(1), port(2)] } }))).toThrow(
      /ports 1, 2/,
    );
    expect(() =>
      selectExposed(model({ a: { image: "n", "x-gangway": { expose: true } } })),
    ).toThrow(/declares no port/);
  });

  test("expose:false opts a publishing service out of the default", () => {
    const m = model({
      db: { image: "pg", ports: [port(5432)], "x-gangway": { expose: false } },
      web: { image: "n", ports: [port(80)] },
    });
  ...
  test("explicit: x-gangway.port wins, expose: entries count, one primary at most", () => {
    const m = model({
      ...
      web: {
        ...
        "x-gangway": { expose: true, primary: true, port: 80 },
      },
...
describe("buildStack", () => {
  const input = resolved(
    {
      web: {
        ...
        labels: {
          "traefik.enable": "true",
          "gangway.managed": "true",
          "gangway.preview_id": "SOMEONE-ELSES",
        },
      },
      db: {
        image: "pg",
        ports: [port(5432)],
        "x-gangway": { expose: false },
        environment: ["POSTGRES_DB=app", "BARE"],
      },
    },
  ...
  test("an unrouted service is OWNED but not MANAGED, so the reconciler never mistakes it for an orphan", () => {
    expect(doc.services.db.labels).toEqual({
      ...
      "gangway.env": "dev",
      "gangway.project": "gw-acme",
      "gangway.preview_id": "01J0000000000000000000000A",
    });
  ...
  test("public URL env wins over the file's own, with $ escaped against interpolation", () => {
    expect(doc.services.web.environment).toMatchObject({
      PRICE: "$$5",
      GANGWAY_PREVIEW_ID: "01J0000000000000000000000A",
      GANGWAY_URL_WEB: "https://acme.preview.example.com:8443",
      PUBLIC_URL: "https://acme.preview.example.com:8443",
    });
...
test("composeForImage is a one-service stack that round-trips through the model", () => {
  ...
  expect(selectExposed(m)).toEqual([
    { service: "web", containerPort: 80, subdomain: null, primary: false },
  ]);
});
...
describe("policy: nothing a build or a secret reads may come from outside the upload", () => {
  ...
  test("a build context inside the source is fine, with or without a dockerfile", () => {
    ...
    expect(
      violations({
        web: { build: { context: "/x/services/web", dockerfile: "docker/Dockerfile.prod" } },
      }),
    ).toEqual([]);
    expect(
      violations({ web: { build: { context: "/x", dockerfile_inline: "FROM scratch" } } }),
    ).toEqual([]);
  });
  ...
  test.each([
    ...
    expect(violations({ web: { build } })[0]).toContain(
      "build.context must be a directory inside the uploaded source",
    );
  });
  ...
  test("secrets and configs: inline content only -- `file` reads the server's disk, `environment` reads gangway's own env", () => {
    expect(
      violations(
        { web: { image: "nginx" } },
        { secrets: { ok: { content: "hunter2" } }, configs: { ok: { content: "a: 1" } } },
      ),
    ).toEqual([]);
    const bad = violations(
      { web: { image: "nginx" } },
      {
        secrets: {
          f: { file: "/x/secret.txt" },
          e: { environment: "GANGWAY_ADMIN_TOKEN" },
          x: { external: true },
        },
