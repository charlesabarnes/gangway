...
function raw(opts: { host: string; path: string; method?: string; headers?: Record<string, string>; body?: string }) {
  return new Promise<{ status: number; headers: Record<string, string>; body: Buffer }>((resolve, reject) => {
    const s = tls.connect(
      { host: "127.0.0.1", port: listener.port, servername: opts.host, rejectUnauthorized: false },
      () => {
        const h: Record<string, string> = { host: opts.host, connection: "close", ...(opts.headers ?? {}) };
...
beforeAll(async () => {
  upstream = Bun.serve({
    ...
    fetch(req, server) {
      ...
      if (url.pathname.startsWith("/echo")) {
        ...
        return Response.json({ host: req.headers.get("host"), path: url.pathname + url.search, headers: h });
      }
      ...
      if (url.pathname === "/sse") {
        ...
        return new Response(new ReadableStream({
          start(c) { t = setInterval(() => { try { c.enqueue(new TextEncoder().encode(`data: ${n++}\n\n`)); } catch { clearInterval(t); } }, 100); },
          cancel() { clearInterval(t); },
        }), { headers: { "content-type": "text/event-stream" } });
      }
      if (url.pathname === "/slow") { server.timeout(req, 0); return Bun.sleep(30_000).then(() => new Response("late")); }
    ...
    websocket: { maxPayloadLength: 8 * 1024 * 1024, message(ws, m) { ws.send(m); }, close() {} },
  });
  ...
  const deps: DispatchDeps = {
    baseDomain: () => BASE,
    table: { lookup: (h: string) => (h === PREVIEW ? entry : undefined) } as unknown as DispatchDeps["table"],
  ...
  listener = startListener({
    hostname: "127.0.0.1", port: 0, maxRequestBodySize: 64 * 1024 * 1024,
    idleTimeout: 10, certStore: store, deps,
  });
});

afterAll(() => { listener?.stop(true); upstream?.stop(true); });
...
describe("headers (§6.4)", () => {
  ...
  test("client-supplied X-Forwarded-* are stripped, not appended to", async () => {
    const r = await raw({ host: PREVIEW, path: "/echo",
      headers: { "x-forwarded-proto": "http", "x-forwarded-for": "6.6.6.6", "x-forwarded-host": "evil.com" } });
    const j = JSON.parse(r.body.toString());
    expect(j.headers["x-forwarded-proto"]).toBe("https");
  ...
  test("unlisted previews get X-Robots-Tag, public ones do not", async () => {
    ...
    expect((await raw({ host: PREVIEW, path: "/echo" })).headers["x-robots-tag"]).toContain("noindex");
...
describe("TLS", () => {
  test("SNI presents the wildcard certificate", async () => {
    const serial = await new Promise<string>((res, rej) => {
      const s = tls.connect({ host: "127.0.0.1", port: listener.port, servername: PREVIEW, rejectUnauthorized: false },
        () => { const c = s.getPeerCertificate(); res(c.serialNumber); s.destroy(); });
      s.on("error", rej);
    });
  ...
  test("the leaf chains to the dev CA, so a client trusting the CA verifies it", async () => {
    const ok = await new Promise<boolean>((res) => {
      const s = tls.connect({ host: "127.0.0.1", port: listener.port, servername: PREVIEW, ca: [caPem] },
        () => { res(s.authorized); s.destroy(); });
  ...
  test("hot-swap: a NEW connection sees the new certificate, via SO_REUSEPORT rebind", async () => {
    ...
    const serialOf = () => new Promise<string>((res, rej) => {
      const s = tls.connect({ host: "127.0.0.1", port: listener.port, servername: PREVIEW, rejectUnauthorized: false },
        () => { res(s.getPeerCertificate().serialNumber); s.destroy(); });
      s.on("error", rej);
    });
...
describe("streaming and limits", () => {
  test("SSE is not buffered and outlives the idle timeout", async () => {
    const res = await fetch(`https://127.0.0.1:${listener.port}/sse`, {
      headers: { host: PREVIEW }, tls: { rejectUnauthorized: false },
    } as RequestInit);
  ...
  test("a body over the cap is 413, and one under it streams through", async () => {
    const ok = await raw({ host: PREVIEW, path: "/count", method: "POST", body: "x".repeat(64 * 1024) });
    expect(JSON.parse(ok.body.toString()).bytes).toBe(64 * 1024);

    const big = await raw({ host: PREVIEW, path: "/count", method: "POST", body: "x".repeat(5 * 1024 * 1024) });
...
describe("WebSocket relay (§6.4 -- fails silently when wrong)", () => {
  test("echoes text and binary, negotiates a subprotocol, propagates close", async () => {
    const ws = new WebSocket(`wss://127.0.0.1:${listener.port}/ws`, {
      protocols: ["gangway-v1"], headers: { host: PREVIEW }, tls: { rejectUnauthorized: false },
    } as never);
    ws.binaryType = "arraybuffer";

    const opened = await new Promise<boolean>((res) => {
      ws.onopen = () => res(true); ws.onerror = () => res(false);
    ...
    const text = await new Promise<boolean>((res) => {
      ws.onmessage = (e) => { if (e.data !== `m${n}`) return res(false); if (++n >= 200) return res(true); ws.send(`m${n}`); };
      ws.send("m0");
    ...
    const binary = await new Promise<boolean>((res) => {
      ws.onmessage = (e) => { const b = new Uint8Array(e.data as ArrayBuffer); res(b.byteLength === big.byteLength && b[0] === 7); };
      ws.send(big);
      setTimeout(() => res(false), 10_000);
    });
