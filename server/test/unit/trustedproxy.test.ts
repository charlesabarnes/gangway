...
describe("clientIpResolver", () => {
  ...
  test("a visitor who comes THROUGH the proxy cannot spoof either: read right to left, stop at the first untrusted hop", () => {
    ...
    expect(clientIpResolver([...NPM, "192.0.2.0/24"])("172.17.0.2", "6.6.6.6, 198.51.100.7, 192.0.2.10")).toBe("198.51.100.7");
  });
...
describe("parseTrustedProxies / config", () => {
  test("accepts bare addresses and CIDRs in both families", () => {
    ...
    expect([l.check("172.17.0.1"), l.check("172.17.0.2"), l.check("10.9.9.9"), l.check("fd12::1", "ipv6")]).toEqual([true, false, true, true]);
  });

  test.each(["npm", "172.17.0.0/33", "172.17.0.0/x", "fd00::/129", ""])("%p is refused loudly -- a typo must not silently mean 'trust nobody'", (bad) => {
  ...
  test("GANGWAY_TRUSTED_PROXIES is a comma-separated list; unset is empty", () => {
    ...
    expect(loadConfig({ GANGWAY_TRUSTED_PROXIES: "172.17.0.0/16, 127.0.0.1" }, {}).trustedProxies).toEqual(["172.17.0.0/16", "127.0.0.1"]);
    expect(loadConfig({}, { trustedProxies: ["10.0.0.0/8"] }).trustedProxies).toEqual(["10.0.0.0/8"]);
  });
});
