...
import { chainVerifiers, staticTokenVerifier, workflowActor, type Actor } from "../../src/auth/actor.ts";
...
const wf = (repo = "acme/web-app", ref = "refs/pull/7/merge", event = "pull_request") => `wf:${repo}:${event}:${ref}`;
...
function make() {
  ...
  s.ctx.policy = new PolicyResolver({
    templates, project: (ref) => projects.find(ref), defaultFor: () => "default",
    projectForSource: (src) => (src.kind === "pushed" ? projects.getByFullName("github", src.pr.repo) : src.kind === "pr" ? projects.getByFullName("github", src.repo) : undefined),
  });
  const secrets = new Secrets(projects, new MemorySettingsStore(), new SecretBox(randomBytes(32)), s.ctx.audit);
  s.ctx.secretsFor = (id, clearance) => secrets.valuesFor(id, clearance);
  const pulls = new Pulls({
    projects,
    previews: { deploy: (i) => deploy(s.ctx, i), destroy: (id, a) => destroy(s.ctx, id, a), findPullRequest: (r, n) => s.ctx.previews.findPullRequest(r, n) },
  });
  const verifyWorkflow = (presented: string): Actor | null => {
    const m = /^wf:([^:]+):([^:]+):(.+)$/.exec(presented);
    return m ? workflowActor({ repository: m[1]!, eventName: m[2]!, ref: m[3]!, runId: "42", actor: "dev" }) : null;
  };
  const auth = { verifyToken: chainVerifiers(staticTokenVerifier(ADMIN), verifyWorkflow), originFor: (h: string) => `https://${h}` };
  const hono = createApp({
    ...
    v1: (api) => {
      ...
      projectRoutes(api, { projects, audit: s.ctx.audit!, secrets, templates, pulls, apiOrigin: () => "https://api.preview.localhost:8443", wire: (p) => ({ ...p, urls: urlsFor(s.ctx, p.id) }) });
    },
    publicV1: (pub) => authRoutes(pub, { auth, accounts: null as never, bootstrap: new Bootstrap(() => 1), roles: null as never, sessionMaxAgeSec: 60 }),
  });
  ...
  const call = (path: string, o: { method?: string; json?: unknown; as?: string } = {}) => {
    ...
    return Promise.resolve(handle(new Request(`https://${HOST}${path}`, { method: o.method ?? "GET", headers, ...(o.json === undefined ? {} : { body: JSON.stringify(o.json) }) }), { clientIp: "203.0.113.7" }));
  };
  const body = (over: Record<string, unknown> = {}) => ({ image: IMAGE, port: 3000, sha: SHA, registry: { username: "dev", password: "ghs_registry_token_value" }, ...over });
...
describe("/v1/projects", () => {
  test("made on purpose: slug from the name, workflow by default; a taken slug or repository is 409; with no repository it is fine", async () => {
    const t = make();
    let res = await t.call("/v1/projects", { method: "POST", json: { name: "Web App", repository: "acme/web-app" } });
    expect(res.status).toBe(201);
    expect((await res.json() as any).project).toMatchObject({ name: "Web App", slug: "web-app", forge: "github", fullName: "acme/web-app", prTrigger: "workflow", enabled: true });
    expect((await t.call("/v1/projects", { method: "POST", json: { name: "web app" } })).status).toBe(409);
    res = await t.call("/v1/projects", { method: "POST", json: { name: "again", repository: "Acme/Web-App" } });
    expect(res.status).toBe(409);
    expect((await t.call("/v1/projects", { method: "POST", json: { name: "x", repository: "not a repo" } })).status).toBe(422);
    expect((await t.call("/v1/projects", { method: "POST", json: { name: "x", templateId: "ghost" } })).status).toBe(422);
    res = await t.call("/v1/projects", { method: "POST", json: { name: "whoami" } });
    expect((await res.json() as any).project).toMatchObject({ slug: "whoami", forge: null, fullName: null });
    expect((await (await t.call("/v1/projects")).json() as any).projects.map((p: any) => p.slug)).toEqual(["web-app", "whoami"]);
  ...
  test("tuned by id or slug; the repository can be changed or removed; delete leaves previews running, unowned", async () => {
    const t = make();
    const p = t.projects.create({ id: "P1", name: "web", slug: "web", forge: "github", fullName: "acme/web-app" });
    let res = await t.call("/v1/projects/web", { method: "PATCH", json: { name: "Web", prTrigger: "webhook", visibility: "private", templateId: null, repository: "acme/web" } });
    expect(res.status).toBe(200);
    expect((await res.json() as any).project).toMatchObject({ name: "Web", prTrigger: "webhook", visibility: "private", fullName: "acme/web" });
    expect((await t.call(`/v1/projects/${p.id}`, { method: "PATCH", json: { repository: null } })).status).toBe(200);
    expect(t.projects.get(p.id)).toMatchObject({ forge: null, fullName: null });
    expect((await t.call("/v1/projects/web", { method: "PATCH", json: { ttl: "soon" } })).status).toBe(422);
    expect((await t.call("/v1/projects/nope", { method: "PATCH", json: { name: "x" } })).status).toBe(404);
  ...
  test("the workflow file names this project and our API, and keeps GitHub's `${{ }}` intact", async () => {
    const t = make();
    t.projects.create({ id: "P1", name: "Web app", slug: "web-app", forge: "github", fullName: "acme/web-app" });
...
describe("/v1/projects/:ref/pulls/:n -- from the project's own workflow", () => {
  ...
  test("deploys the pushed image as the PR's preview, filed under the project, with its secrets as env; the registry login lives for `up` only", async () => {
    ...
    const res = await t.call("/v1/projects/web-app/pulls/7?wait=true", { method: "PUT", as: wf(), json: t.body() });
    expect(res.status).toBe(201);
    const { preview } = await res.json() as any;
    expect(preview).toMatchObject({ state: "awake", projectId: "P1", templateId: "default", source: { kind: "pr", repo: "acme/web-app", number: 7, sha: SHA, image: IMAGE } });
    expect(preview.urls[0].url).toMatch(/^https:\/\/web-app-pr-7-[a-z0-9]+\.preview\.localhost:8443\/$/);
    const login = t.s.fake.upLogins[0]!;
    expect(JSON.parse(login.config!)).toEqual({ auths: { "ghcr.io": { auth: Buffer.from("dev:ghs_registry_token_value").toString("base64") } } });
    ...
    const everywhere = JSON.stringify([t.s.previews.get(preview.id), t.s.ctx.logs.tail(preview.id), t.s.audit.page({ limit: 20 }).entries]);
  ...
  test("the same head again is left alone; a new head replaces the preview; close tears it down with its image, and closing again is fine", async () => {
    const t = setup();
    const first = await (await t.call("/v1/projects/web-app/pulls/7?wait=true", { method: "PUT", as: wf(), json: t.body() })).json() as any;
    const again = await t.call("/v1/projects/web-app/pulls/7?wait=true", { method: "PUT", as: wf(), json: t.body() });
    expect(again.status).toBe(200);
    expect((await again.json() as any)).toMatchObject({ unchanged: true, preview: { id: first.preview.id } });
    const next = await (await t.call("/v1/projects/web-app/pulls/7?wait=true", { method: "PUT", as: wf(), json: t.body({ sha: "b".repeat(40), image: IMAGE.replace(/d+$/, "e".repeat(64)) }) })).json() as any;
    ...
    expect((await t.call("/v1/projects/web-app/pulls/7", { method: "DELETE", as: wf() })).status).toBe(204);
  ...
  test("a workflow acts only for its own repository, its own PR, on a pull_request event, for a workflow-mode project", async () => {
    const t = setup();
    const put = (as: string, path = "/v1/projects/web-app/pulls/7") => t.call(path, { method: "PUT", as, json: t.body() });
    ...
    expect((await put(wf("acme/web-app", "refs/pull/7/merge", "pull_request_target"))).status).toBe(403);
  ...
  test("a workflow token reaches nothing else: not the preview list, not a plain deploy, not the project's settings", async () => {
    ...
    expect((await t.call("/v1/previews", { method: "POST", as: wf(), json: { source: { kind: "image", image: "nginx", port: 80 } } })).status).toBe(403);
    ...
    expect((await t.call("/v1/projects/web-app/pulls/7/extra", { method: "PUT", as: wf() })).status).toBe(403);
  });

  test("an admin may do the same by hand; a project with no repository has no pull requests", async () => {
    const t = setup();
    const res = await t.call("/v1/projects/web-app/pulls/3", { method: "PUT", json: t.body({ registry: undefined }) });
    ...
    expect((await t.call("/v1/projects/bare/pulls/3", { method: "PUT", json: t.body() })).status).toBe(422);
    expect((await t.call("/v1/projects/web-app/pulls/0", { method: "PUT", json: t.body() })).status).toBe(400);
    expect((await t.call("/v1/projects/web-app/pulls/3", { method: "PUT", json: t.body({ image: "not an image!" }) })).status).toBe(422);
  });
  ...
  test("a plain deploy can name a project: it is filed under it and follows its policy", async () => {
    ...
    const res = await t.call("/v1/previews?wait=true", { method: "POST", json: { name: "adhoc", project: "web-app", source: { kind: "image", image: "traefik/whoami:v1.10", port: 80 } } });
    expect(res.status).toBe(201);
    expect((await res.json() as any).preview).toMatchObject({ projectId: "P1", visibility: "public" });
    expect((await t.call("/v1/previews", { method: "POST", json: { project: "ghost", source: { kind: "image", image: "x", port: 80 } } })).status).toBe(422);
  });
