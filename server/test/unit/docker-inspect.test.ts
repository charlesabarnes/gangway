...
import {
  classifyContainer, containerName, findPublishedPort, healthState, isReady, isRunning,
  parsePortKey, portDrift, publishedPorts, scanManaged, type InspectJson,
...
describe("publishedPorts", () => {
  test("flattens every binding and skips unpublished ports", () => {
    ...
    expect(ports.filter((p) => p.containerPort === 8080).map((p) => p.hostIp).sort())
      .toEqual(["0.0.0.0", "::"]);
  });
  ...
  test("a malformed HostPort is dropped rather than becoming NaN", () => {
    const ports = publishedPorts({
      NetworkSettings: { Ports: { "80/tcp": [{ HostIp: "0.0.0.0", HostPort: "" }, { HostIp: "0.0.0.0" }] } },
    });
...
describe("findPublishedPort", () => {
  /* dockerd binds both families when no bind address is given. Returning whichever the
     daemon listed first would hand the proxy "::" half the time. */
...
describe("portDrift (§11 row 1: verify port, continue)", () => {
  ...
  test("a container that published nothing is distinguishable from one that moved", () => {
    expect(portDrift(route, { NetworkSettings: { Ports: { "8080/tcp": null } } }))
      .toEqual({ kind: "no-binding", expected: 31042 });
  });
});
...
const summary = (over: Partial<ContainerSummary> = {}): ContainerSummary => ({
  id: "c0ffee", names: ["gw-acme-pr-123-api-1"], image: "gw/acme:pr-123",
  state: "running", status: "Up 2 minutes", labels: buildLabels(labels),
  ports: [], createdAt: new Date("2026-02-03T04:05:06.000Z"), ...over,
});
...
describe("scanning a daemon for our containers", () => {
  ...
  test("future-version and malformed stay separable in the scan output", async () => {
    ...
    const future = summary({
      id: "beef", labels: { ...buildLabels(labels), "gangway.version": "9" },
    });
    const broken = summary({
      id: "dead", labels: { "gangway.managed": "true", "gangway.version": "1" },
    });
