...
import { ALL_PERMISSIONS, DEFAULT_ROLE_PERMISSIONS, PERMISSIONS } from "../../../shared/src/permissions.ts";
...
for (const [name, open] of DRIVERS) {
  describe(`account repos (${name})`, () => {
    ...
    const setup = () => {
      ...
      const ada = () => users.create({ id: "u-ada", email: "ada@example.com", roleId: "admin", hash: "scrypt$x", salt: "s" });
      return { db, users, ada, sessions: new SessionsRepo(db, now), tokens: new TokensRepo(db, now), audit: new AuditRepo(db, now), roles: new RolesRepo(db) };
    };
    ...
    describe("users", () => {
      test("a User never carries password material; credentials() is the only way to it", () => {
        ...
        const u = ada();
        expect(u).toEqual({ id: "u-ada", email: "ada@example.com", roleId: "admin", disabled: false, createdAt: new Date(clock) });
        expect(JSON.stringify([u, users.list(), users.getByEmail("ada@example.com")])).not.toContain("scrypt");
      ...
      test("email is unique, and a role must exist", () => {
        const { users, ada } = setup();
        ada();
        expect(() => users.create({ id: "u-2", email: "ada@example.com", roleId: "viewer", hash: "h", salt: "s" })).toThrow();
        expect(() => users.create({ id: "u-3", email: "bob@example.com", roleId: "wizard", hash: "h", salt: "s" })).toThrow();
      ...
      test("countActiveAdmins answers the last-admin question", () => {
        const { users, ada } = setup();
        ada();
        users.create({ id: "u-bob", email: "bob@example.com", roleId: "member", hash: "h", salt: "s" });
      ...
      test("update touches only what it is given", () => {
        const { users, ada } = setup();
        ada();
        expect(users.update("u-ada", { disabled: true })).toMatchObject({ roleId: "admin", disabled: true });
        expect(users.update("u-ada", { roleId: "viewer" })).toMatchObject({ roleId: "viewer", disabled: true });
        expect(users.update("nope", { disabled: true })).toBeUndefined();
      });
    });

    describe("sessions", () => {
      const open1 = (s: ReturnType<typeof setup>) => { s.ada(); return s.sessions.create({ id: "h1", userId: "u-ada", expiresAt: clock + 7 * DAY, ip: "::1", userAgent: "curl" }); };
      ...
      test("sliding never passes the absolute cap", () => {
        ...
        expect(s.sessions.touch("h1", { staleBefore: clock, idleMs: 7 * DAY, absoluteMs: 30 * DAY })).toBe(true);
      ...
      test("deleteForUser can spare the current session; purgeExpired takes only the dead", () => {
        const s = setup();
        open1(s);
        s.sessions.create({ id: "h2", userId: "u-ada", expiresAt: clock + DAY, ip: null, userAgent: null });
        s.sessions.create({ id: "h3", userId: "u-ada", expiresAt: clock + MIN, ip: null, userAgent: null });
        clock += 2 * MIN;
    ...
    describe("tokens", () => {
      const mint = (s: ReturnType<typeof setup>, over: Partial<Parameters<TokensRepo["create"]>[0]> = {}) =>
        s.tokens.create({ id: "t1", name: "ci", prefix: "gw_abcdefgh", tokenHash: "hash-1", scopes: ["deploy"], userId: "u-ada", expiresAt: null, ...over });

      test("the hash never appears in a domain object", () => {
        const s = setup();
        s.ada();
        const t = mint(s);
        expect(t).toMatchObject({ id: "t1", name: "ci", prefix: "gw_abcdefgh", scopes: ["deploy"], userId: "u-ada", revokedAt: null, lastUsedAt: null });
        expect(JSON.stringify([t, s.tokens.listAll(), s.tokens.listForUser("u-ada"), s.tokens.findActiveByHash("hash-1")])).not.toContain("hash-1");
      });
      ...
      test("findActiveByHash returns the owner, and refuses revoked, expired and disabled-owner tokens", () => {
        ...
        expect(s.tokens.findActiveByHash("hash-1")!.owner).toMatchObject({ id: "u-ada", roleId: "admin" });
      ...
      test("an ownerless token is active with a null owner", () => {
        ...
        expect(s.tokens.findActiveByHash("hash-1")).toMatchObject({ owner: null, token: { scopes: ["admin"] } });
      });
    ...
    describe("audit", () => {
      test("pages newest-first with a cursor, and filters by action", () => {
        const { audit } = setup();
        for (let i = 1; i <= 5; i++) { clock += 1; audit.append({ actorType: "user", actorId: "u-ada", action: i % 2 ? "auth.login" : "preview.deploy", target: `t${i}`, new: { i } }); }

        const first = audit.page({ limit: 2 });
        expect(first.entries.map((e) => e.target)).toEqual(["t5", "t4"]);
        expect(first.entries[0]).toMatchObject({ actorType: "user", actorId: "u-ada", old: null, new: { i: 5 } });
        ...
        expect(audit.page({ limit: 10, action: "auth.login" }).entries.map((e) => e.target)).toEqual(["t5", "t3", "t1"]);
      });

      test("an actor type the schema does not know is refused", () => {
        const { audit } = setup();
        expect(() => audit.append({ actorType: "martian" as never, actorId: null, action: "x", target: null })).toThrow();
      });
    });
    ...
    describe("roles and the permission matrix", () => {
      test("the seeded matrix is what the code's defaults say, and admin is everything", () => {
        ...
        expect(rp.roles().map((r) => [r.id, r.builtin, r.editable])).toEqual([["admin", true, false], ["member", true, true], ["viewer", true, true]]);
      });
      ...
      test("an edit replaces the set, reports old and new, and survives a reload from SQLite", () => {
        ...
        expect([...new RolePermissions(roles).for("viewer")].sort()).toEqual(["audit.read", "previews.read"]);
        rp.set("viewer", []);
      ...
      test("syncCatalogue adds what the code has learned, grants it to admin, and leaves the operator's matrix alone", () => {
        ...
        const grown = [...PERMISSIONS, { id: "previews.clone", feature: "previews", description: "a later phase" }];
        expect(roles.syncCatalogue(grown).added).toEqual(["previews.clone"]);
        expect(roles.syncCatalogue(grown).added).toEqual([]);
        expect(db.get<{ ok: number }>("SELECT 1 AS ok FROM role_permissions WHERE role_id = 'admin' AND permission_id = 'previews.clone'")).toEqual({ ok: 1 });
        rp.reload();
      ...
      test("a grant on an id the code does not know is inert: it is never served", () => {
        ...
        db.run("INSERT INTO role_permissions (role_id, permission_id) VALUES ('viewer', 'previews.retired')");
