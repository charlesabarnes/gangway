...
function setup(o: { maxBytes?: number } = {}) {
  ...
  const uploads = new Uploads({ dir, url: (id) => `https://mcp.preview.localhost:8443/uploads/${id}`, now: s.ctx.now, ...o });
  const tools = new Tools({ ctx: s.ctx, deploys: new IdempotentDeploys(s.ctx, new IdempotencyRepo(s.db, s.ctx.now)), uploads, logger: quiet });
  const mcp = new McpSurface({ tools, uploads, verifyToken: staticTokenVerifier(TOKEN), logger: quiet }).handler();
  const scope = (actor: Actor = ACTOR): CallScope => ({ actor, signal: new AbortController().signal });
  const put = (url: string, body: Uint8Array | null, headers: Record<string, string> = {}) =>
    Promise.resolve(mcp(new Request(url, { method: "PUT", body, headers: { "content-type": "application/gzip", ...headers } }), { clientIp: "203.0.113.9" }));
...
describe("upload by reference", () => {
  test("new -> PUT -> deploy: the preview is exactly the uploaded bytes, and nothing is left on disk", async () => {
    ...
    const out = await s.tools.deploy(s.scope(), { upload: s.idOf(offer), name: "big", visibility: "public" });
    ...
    expect((await s.ctx.sources!.list(id)).files.map((f) => f.path)).toEqual(["css/a.css", "index.html"]);
    expect(readdirSync(s.dir)).toEqual([]);
    // One use.
    await expect(s.tools.deploy(s.scope(), { upload: s.idOf(offer), name: "again", visibility: "public" })).rejects.toMatchObject({ code: "not_found" });
  ...
  test("preview + upload replaces the whole source and rebuilds at the same URL", async () => {
    const s = setup();
    await s.tools.deploy(s.scope(), { files: { "index.html": "v1", "old.html": "gone soon" }, name: "site", visibility: "public" });
    const offer = await s.tools.deploy(s.scope(), { upload: "new" });
    ...
    expect((await s.ctx.sources!.list(resolvePreview(s.ctx, "site").id)).files.map((f) => f.path)).toEqual(["index.html"]);
    await expect(s.tools.deploy(s.scope(), { preview: "site", upload: "x", files: { a: "" } })).rejects.toThrow("one or the other");
  });

  test("only the credential that asked may deploy it; nothing sent yet is a clear error", async () => {
    const s = setup();
    const offer = await s.tools.deploy(s.scope(), { upload: "new" });
    await expect(s.tools.deploy(s.scope(), { upload: s.idOf(offer), name: "early" })).rejects.toThrow("nothing has been sent");
    await s.put(s.urlOf(offer), (await packFiles({ "index.html": "x" })).archive);
    const stranger = tokenActor("t-other", ["deploy"]) as Actor;
    await expect(s.tools.deploy(s.scope(stranger), { upload: s.idOf(offer), name: "stolen" })).rejects.toMatchObject({ code: "not_found" });
    expect(await s.tools.deploy(s.scope(), { upload: s.idOf(offer), name: "mine", visibility: "public" })).toStartWith("ready:");
  });
  ...
  test("the PUT: a cap on bytes, no browsers, no second PUT, unknown ids 404, and a read credential cannot ask", async () => {
    ...
    expect((await s.put(url, new Uint8Array(10), { origin: "https://evil.example" })).status).toBe(403);
    expect((await s.put(url, new Uint8Array(10))).status).toBe(201);
    expect((await s.put(url, new Uint8Array(10))).status).toBe(409);
    expect((await s.put(url.replace(/[^/]+$/, "A".repeat(43)), new Uint8Array(10))).status).toBe(404);
    expect((await s.put(url.replace(/[^/]+$/, "..%2F..%2Fetc"), new Uint8Array(10))).status).toBe(404);
    await expect(s.tools.deploy(s.scope(tokenActor("t-r", ["read"]) as Actor), { upload: "new" })).rejects.toThrow('"previews.deploy"');
  });
  ...
  test("a slot expires, and its bytes go with it", async () => {
    ...
    await expect(s.tools.deploy(s.scope(), { upload: s.idOf(offer), name: "late" })).rejects.toMatchObject({ code: "not_found" });
    expect(readdirSync(s.dir)).toEqual([]);
  });

  test("without an upload store, upload is refused and files still work", async () => {
    const s = setupPreviewContext();
    const tools = new Tools({ ctx: s.ctx, deploys: new IdempotentDeploys(s.ctx, new IdempotencyRepo(s.db, s.ctx.now)), logger: quiet });
    await expect(tools.deploy({ actor: ACTOR, signal: new AbortController().signal }, { upload: "new" })).rejects.toThrow("send files instead");
  });
});
