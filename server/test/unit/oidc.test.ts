...
function keypair(kid: string) {
  ...
  return { kid, privateKey, jwk: { ...(publicKey.export({ format: "jwk" }) as object), kid, alg: "RS256", use: "sig" } };
}

function sign(k: { kid: string; privateKey: KeyObject }, claims: Record<string, unknown>, header: Record<string, unknown> = {}): string {
  const enc = (o: object) => Buffer.from(JSON.stringify(o)).toString("base64url");
  const head = enc({ alg: "RS256", typ: "JWT", kid: k.kid, ...header });
  const body = enc(claims);
  const sig = createSign("RSA-SHA256").update(`${head}.${body}`).sign(k.privateKey).toString("base64url");
  return `${head}.${body}.${sig}`;
}

const claims = (over: Record<string, unknown> = {}) => ({
  iss: GITHUB_ACTIONS_ISSUER, aud: AUD, iat: NOW / 1000 - 5, nbf: NOW / 1000 - 5, exp: NOW / 1000 + 300,
  repository: "acme/web-app", repository_id: "123", event_name: "pull_request", ref: "refs/pull/7/merge", sha: "b".repeat(40), run_id: "999", actor: "dev",
  ...over,
});
...
function setup() {
  ...
  const oidc = new GitHubOidc({
    audience: () => AUD, now: () => clock,
    fetch: async (url) => { fetches.push(url); return new Response(JSON.stringify(served), { status: 200 }); },
  });
  return { a, served, fetches, oidc, advance: (ms: number) => { clock += ms; } };
}
...
describe("GitHubOidc", () => {
  test("a genuine token gives the run's claims; the key set is fetched once and cached", async () => {
    ...
    expect(c).toEqual({ repository: "acme/web-app", repositoryId: "123", eventName: "pull_request", ref: "refs/pull/7/merge", sha: "b".repeat(40), runId: "999", actor: "dev" });
  ...
  test("refused: another audience, another issuer, expired, not yet valid, a bad repository claim, alg none, a forged signature", async () => {
    ...
    expect(await t.oidc.verify(sign(t.a, claims({ aud: "https://api.elsewhere.example" })))).toBeNull();
  ...
  test("a stranger's token or a gangway token never makes us fetch keys", async () => {
    ...
    expect(await t.oidc.verify(sign(t.a, claims({ iss: "https://accounts.google.com" })))).toBeNull();
  ...
  test("workflowActor: the PR number comes from the ref; a push has none", () => {
    expect(workflowActor({ repository: "acme/web-app", runId: "1", actor: "dev", eventName: "pull_request", ref: "refs/pull/7/merge" })).toMatchObject({ kind: "workflow", pull: 7 });
    expect(workflowActor({ repository: "acme/web-app", runId: "1", actor: "dev", eventName: "push", ref: "refs/heads/main" })).toMatchObject({ pull: null });
  });
});
