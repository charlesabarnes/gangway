...
describe("jittered", () => {
  test("spreads symmetrically around the interval and never goes negative", () => {
    ...
    expect(jittered(1000, 5, () => 0)).toBe(0);
  });
});

describe("Scheduler", () => {
  test("start(), and only start(), arms each enabled job with a jittered delay", () => {
    const h = harness(() => 1);
  ...
  test("arms the next run only when the previous one settles, so a job never overlaps", async () => {
    const h = harness();
    const gate = deferred();
    let concurrent = 0,
      peak = 0;
  ...
  test("a throwing job is logged, counted and rescheduled; trigger() rethrows", async () => {
    const h = harness();
    let n = 0;
    h.scheduler.register({
      name: "flaky",
  ...
  test("stop cancels timers, aborts the signal, and waits for the running job", async () => {
    ...
    expect(h.scheduler.stop()).toBe(h.scheduler.stop());
    await tick();
    expect(signal!.aborted).toBe(true);
    expect(stopped).toBe(false);
    expect(h.delays()).toEqual([9999]);
    gate.resolve();
    await stopping;
