...
describe("the tools", () => {
  ...
  test("bad input is refused before anything starts", async () => {
    const s = setup();
    await expect(s.tools.deploy(s.scope(), {})).rejects.toThrow(
      "exactly one of files, upload, image or git",
    );
    await expect(s.tools.deploy(s.scope(), { image: "nginx" })).rejects.toThrow("needs port");
    await expect(
      s.tools.deploy(s.scope(), { image: "nginx", port: 80, addons: ["postgres"] }),
    ).rejects.toThrow("addons go with files");
    await expect(
      s.tools.deploy(s.scope(), { files: { a: "" }, addons: ["mongo"] }),
    ).rejects.toThrow();
    await expect(s.tools.deploy(s.scope(), { files: { a: "" }, remove: ["b"] })).rejects.toThrow(
  ...
  test("ready says how the upload was read, hashes what is deployed, and GETs the paths asked for", async () => {
    ...
    expect(again).toContain(
      `${createHash("sha256").update("v2").digest("hex").slice(0, 12)}         2  index.html`,
    );
...
function surface() {
  ...
  const messages = async (res: Response) =>
    (await res.text())
      ...
      .map(
        (l) =>
          JSON.parse(l.slice(6)) as { id?: number; result?: any; error?: any; method?: string },
      );
