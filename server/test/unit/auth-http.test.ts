...
function make() {
  ...
  const app = createApp({
    ...
    v1: (api) => {
      api.get("/whoami", requirePermission("previews.read"), (c) => c.json({ id: actorId(c.get("actor")) }));
      api.post("/mutate", requirePermission("previews.deploy"), (c) => c.json({ ok: true }));
    },
    publicV1: (pub) => authRoutes(pub, { auth, accounts: s.accounts, bootstrap, roles: s.roles, sessionMaxAgeSec: 2_592_000 }),
  });
  const on = (surface: "app" | "api") => {
    ...
    return (path: string, init: RequestInit & { json?: unknown } = {}) => {
      ...
      return Promise.resolve(h(new Request(`https://${host}${path}`, { ...init, headers, ...(init.json === undefined ? {} : { body: JSON.stringify(init.json) }) }), { clientIp: "203.0.113.7" }));
    };
  };
...
async function loggedIn() {
  const t = make();
  const res = await t.app("/v1/auth/setup", { method: "POST", json: { token: t.setupToken(), email: "ada@example.com", password: PASSWORD } });
...
describe("first-run setup over HTTP", () => {
  test("session says setupRequired until the first admin exists; setup logs them in", async () => {
    const t = make();
    expect(await (await t.app("/v1/auth/session")).json()).toEqual({ authenticated: false, setupRequired: true });

    const res = await t.app("/v1/auth/setup", { method: "POST", json: { token: t.setupToken(), email: "  Ada@Example.com ", password: PASSWORD } });
    expect(res.status).toBe(201);
    expect(await res.json()).toMatchObject({ user: { email: "ada@example.com", role: { id: "admin", name: "admin" } } });

    const me = await (await t.app("/v1/auth/session", { headers: { cookie: t.cookieOf(res) } })).json() as { permissions: string[] };
    expect(me).toMatchObject({ authenticated: true, setupRequired: false, user: { email: "ada@example.com" } });
  ...
  test("the cookie is __Host-, HttpOnly, Secure, SameSite=Lax, Path=/ and has NO Domain", async () => {
    ...
    expect(setupRes.headers.get("set-cookie")).toMatch(/^__Host-gw_session=[A-Za-z0-9_-]{43}; Max-Age=2592000; Path=\/; HttpOnly; Secure; SameSite=Lax$/);
  });

  test("a wrong link is 403 and does not burn the right one; a weak password is 422 and does not either", async () => {
    const t = make();
    expect((await t.app("/v1/auth/setup", { method: "POST", json: { token: "gw_setup_nope", email: "a@example.com", password: PASSWORD } })).status).toBe(403);
    expect((await t.app("/v1/auth/setup", { method: "POST", json: { token: t.setupToken(), email: "a@example.com", password: "short" } })).status).toBe(422);
    expect((await t.app("/v1/auth/setup", { method: "POST", json: { token: t.setupToken(), email: "a@example.com", password: PASSWORD } })).status).toBe(201);
  });

  test("once an admin exists, setup is a 404 -- even with the right link", async () => {
    const t = make();
    const token = t.setupToken();
    await t.app("/v1/auth/setup", { method: "POST", json: { token, email: "ada@example.com", password: PASSWORD } });
    expect((await t.app("/v1/auth/setup", { method: "POST", json: { token, email: "eve@example.com", password: PASSWORD } })).status).toBe(404);
    expect(t.s.users.count()).toBe(1);
  });

  test("setup and login exist on the app hostname only: that is where the cookie lives", async () => {
    const t = make();
    expect((await t.api("/v1/auth/setup", { method: "POST", json: { token: t.setupToken(), email: "a@example.com", password: PASSWORD } })).status).toBe(404);
    expect((await t.api("/v1/auth/login", { method: "POST", json: { email: "a@example.com", password: PASSWORD } })).status).toBe(404);
  });
});
...
describe("login over HTTP", () => {
  test("right password: a cookie. Wrong: 401 with no cookie, and the same body as an unknown email", async () => {
    ...
    const ok = await t.app("/v1/auth/login", { method: "POST", json: { email: "ada@example.com", password: PASSWORD } });
    expect(ok.status).toBe(200);
    expect(ok.headers.get("set-cookie")).toStartWith("__Host-gw_session=");
    expect(ok.headers.get("cache-control")).toBe("no-store");

    const wrong = await t.app("/v1/auth/login", { method: "POST", json: { email: "ada@example.com", password: "not the password!" } });
    const unknown = await t.app("/v1/auth/login", { method: "POST", json: { email: "nobody@example.com", password: PASSWORD } });
    expect([wrong.status, unknown.status]).toEqual([401, 401]);
    expect(wrong.headers.get("set-cookie")).toBeNull();
    const strip = (b: Record<string, unknown>) => ({ ...b, requestId: null });
    expect(strip(await wrong.json() as Record<string, unknown>)).toEqual(strip(await unknown.json() as Record<string, unknown>));
  });

  test("lockout reaches the client as 429 with Retry-After", async () => {
    const t = await loggedIn();
    for (let i = 0; i < 5; i++) await t.app("/v1/auth/login", { method: "POST", json: { email: "ada@example.com", password: "wrong wrong wrong" } });
    const res = await t.app("/v1/auth/login", { method: "POST", json: { email: "ada@example.com", password: PASSWORD } });
  ...
  test("login CSRF: a page on a sibling preview cannot log your browser into ITS account", async () => {
    ...
    expect((await t.app("/v1/auth/login", { method: "POST", json: body, headers: { origin: SIBLING } })).status).toBe(403);
    expect((await t.app("/v1/auth/login", { method: "POST", json: body, headers: { origin: APP } })).status).toBe(200);
  ...
  test("a non-JSON body is a 400, not a 500", async () => {
    const t = make();
    expect((await t.app("/v1/auth/login", { method: "POST", body: "email=a", headers: { "content-type": "text/plain" } })).status).toBe(400);
  });
});
...
describe("the session cookie as a credential", () => {
  ...
  test("a mutation needs our own Origin: missing, a sibling preview, and same-SITE are all refused", async () => {
    const t = await loggedIn();
    const post = (headers: Record<string, string>) => t.app("/v1/mutate", { method: "POST", headers: { cookie: t.cookie, ...headers } });
  ...
  test("the refusal is a 403, not a 401: the credential was fine, the request was not", async () => {
    const t = await loggedIn();
    const res = await t.app("/v1/mutate", { method: "POST", headers: { cookie: t.cookie, origin: SIBLING } });
  ...
  test("a bearer token needs no Origin, and beats a cookie sent alongside it", async () => {
    const t = await loggedIn();
    expect((await t.api("/v1/mutate", { method: "POST", headers: { authorization: `Bearer ${TOKEN}` } })).status).toBe(200);
    const both = await t.app("/v1/whoami", { headers: { authorization: `Bearer ${TOKEN}`, cookie: t.cookie } });
    expect(await both.json()).toEqual({ id: "env:admin" });
  });

  test("a WRONG bearer never falls through to a valid cookie", async () => {
    const t = await loggedIn();
    expect((await t.app("/v1/whoami", { headers: { authorization: "Bearer gw_wrong_token_0123456789abcdef", cookie: t.cookie } })).status).toBe(401);
  });

  test("a forged or garbage cookie is just unauthenticated", async () => {
    const t = await loggedIn();
    for (const cookie of ["__Host-gw_session=", "__Host-gw_session=nope", `__Host-gw_session=${"A".repeat(43)}`, `gw_session=${t.cookie.split("=")[1]}`]) {
...
describe("logout and changing a password", () => {
  test("logout ends the session server-side, clears the cookie, and is idempotent", async () => {
    const t = await loggedIn();
    const res = await t.app("/v1/auth/logout", { method: "POST", headers: { cookie: t.cookie, origin: APP } });
    expect(res.status).toBe(204);
    expect(res.headers.get("set-cookie")).toMatch(/^__Host-gw_session=; Max-Age=0; Path=\/; Secure$/);
    expect((await t.app("/v1/whoami", { headers: { cookie: t.cookie } })).status).toBe(401);
    expect((await t.app("/v1/auth/logout", { method: "POST", headers: { cookie: t.cookie, origin: APP } })).status).toBe(204);
  });

  test("a sibling preview cannot log you out", async () => {
    const t = await loggedIn();
    expect((await t.app("/v1/auth/logout", { method: "POST", headers: { cookie: t.cookie, origin: SIBLING } })).status).toBe(403);
  ...
  test("change password: needs a session, the current password, and our Origin", async () => {
    ...
    expect((await t.app("/v1/auth/password", { method: "POST", json: body, headers: { cookie: t.cookie, origin: SIBLING } })).status).toBe(403);
    expect((await t.app("/v1/auth/password", { method: "POST", json: { ...body, current: "not my password!" }, headers: { cookie: t.cookie, origin: APP } })).status).toBe(403);
    expect((await t.app("/v1/auth/password", { method: "POST", json: { ...body, next: "short" }, headers: { cookie: t.cookie, origin: APP } })).status).toBe(422);
    expect((await t.app("/v1/auth/password", { method: "POST", json: body, headers: { cookie: t.cookie, origin: APP } })).status).toBe(204);
    expect((await t.app("/v1/auth/login", { method: "POST", json: { email: "ada@example.com", password: "a brand new password" } })).status).toBe(200);
  });

  test("session describes a bearer token too, without pretending it is a person", async () => {
    const t = make();
    expect(await (await t.api("/v1/auth/session", { headers: { authorization: `Bearer ${TOKEN}` } })).json())
