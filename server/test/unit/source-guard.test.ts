...
import { assertNoEscapingSymlinks, inspectComposeFile, referencedFiles } from "../../src/previews/source/guard.ts";
...
describe("assertNoEscapingSymlinks", () => {
  ...
  test.each([
    ...
    await expect(assertNoEscapingSymlinks(dir)).rejects.toMatchObject({ code: "unprocessable", message: expect.stringContaining("src/.env") });
  });
...
describe("referencedFiles", () => {
  test("finds every file `compose config` would open, in every spelling", () => {
    expect(referencedFiles({
      include: ["a.yaml", { path: ["b.yaml", "c.yaml"], env_file: "inc.env", project_directory: "sub" }],
      services: {
        web: { env_file: [".env.web", { path: "opt.env", required: false }], extends: { file: "base.yaml", service: "x" } },
        db: { env_file: "db.env" },
      },
    }).map((r) => r.path)).toEqual(["a.yaml", "b.yaml", "c.yaml", "inc.env", "sub", ".env.web", "opt.env", "base.yaml", "db.env"]);
...
describe("inspectComposeFile", () => {
  test("prefers compose.yaml, accepts the legacy names, and reports none as null", async () => {
    expect(await inspectComposeFile(tree({ "compose.yaml": "services: {}", "docker-compose.yml": "services: {}" }))).toBe("compose.yaml");
    expect(await inspectComposeFile(tree({ "docker-compose.yml": "services: {}" }))).toBe("docker-compose.yml");
  ...
  test.each([
    ...
    const dir = tree({ "compose.yaml": `services:\n  web:\n    image: nginx\n    ${line}\n` });
    await expect(inspectComposeFile(dir)).rejects.toMatchObject({ code: "unprocessable", message: expect.stringContaining(where) });
  });

  test("an out-of-tree include is refused; broken YAML is a 422 that says so", async () => {
    await expect(inspectComposeFile(tree({ "compose.yaml": "include:\n  - ../other/compose.yaml\nservices: {}\n" }))).rejects.toThrow("outside the uploaded source");
    await expect(inspectComposeFile(tree({ "compose.yaml": "services: [unclosed" }))).rejects.toMatchObject({ code: "unprocessable", message: expect.stringContaining("not valid YAML") });
  });
