...
describe("drivers", () => {
  ...
  test('no secret and no query text inside a shell script: the password comes from the container, the text is "$1"', () => {
    ...
    expect(queryArgv("postgres", "select 1", true)).not.toContain(
  ...
  test("table names are quoted by doubling", () => {
    expect(rowsQuery("postgres", { schema: "public", name: 'we"ird' }, 50, 0)).toBe(
      `select * from "public"."we""ird" limit 50 offset 0`,
    );
    expect(rowsQuery("mysql", { schema: "app", name: "a`b" }, 10, 20)).toBe(
      "select * from `app`.`a``b` limit 10 offset 20",
    );
  });

  test("redis: tokenizing, the read allowlist, and what is never allowed", () => {
    expect(tokenize(`HSET "my key" field "va\\"lue"`)).toEqual([
      "HSET",
      "my key",
      "field",
      'va"lue',
...
async function withPostgres(
  exec: (argv: string[], signal?: AbortSignal) => AsyncGenerator<ComposeEvent>,
) {
  ...
  s.ctx.compose = {
    stream: (argv, host, o) =>
      argv.includes("exec") ? (seen.push(argv), exec(argv, o.signal)) : real.stream(argv, host, o),
    capture: async (argv, host, o): Promise<ComposeResult> =>
      argv[1] === "ps" && argv.includes("--quiet")
...
describe("the service", () => {
  test("a query runs in the add-on's container, parses, and is audited without its results", async () => {
    ...
    expect(Object.keys(data.list(p.id)[0]!).sort()).toEqual(
      Object.keys(contract.previewAddons[0]!).sort(),
    );
  ...
  test("one query per preview at a time", async () => {
    ...
    const { p, data } = await withPostgres(async function* () {
      await gate;
      yield { type: "exit", code: 0, signal: null };
    });
    ...
    expect(data.list(p.id)).toEqual([
      {
        ...
        name: "PostgreSQL",
        service: "postgres",
        env: expect.arrayContaining(["DATABASE_URL"]),
      },
    ]);
  });
});
