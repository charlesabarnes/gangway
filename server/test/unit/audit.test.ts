...
describe("the audit log is written by the service layer", () => {
  test("a deploy is recorded once the rows exist, with who, what and where", async () => {
    ...
    expect(all(s.audit)).toEqual([expect.objectContaining({
      ...
      new: { project: "gw-audited", visibility: "public", source: "image", hostId: preview.hostId, urls: ["https://audited.preview.localhost:8443/"] },
    })]);
  });
  ...
  test("a destroy records what the preview WAS, and a user is a user", async () => {
    ...
    const user: Actor = { kind: "user", userId: "u-ada", roleId: "member", permissions: new Set(["previews.destroy"]), sessionId: "sess" };
    await destroy(s.ctx, preview.id, user);
    expect(all(s.audit)[0]).toMatchObject({
      actorType: "user", actorId: "u-ada", action: "preview.destroy", target: preview.id,
      old: { project: "gw-doomed", state: "awake", hostId: preview.hostId }, new: null,
    });
  });
  ...
  test("the TTL sweep's destroy is recorded as the system's, not as nobody's", async () => {
    ...
    s.db.run("UPDATE previews SET ttl_expires_at = $t WHERE id = $id", { t: Date.now() - 1000, id: preview.id });
    await sweepExpired(s.ctx, s.logger);
    expect(all(s.audit)[0]).toMatchObject({ actorType: "system", actorId: "ttl-sweep", action: "preview.destroy", target: preview.id });
  });
});
...
describe("Audit.record", () => {
  test("redacts on the way in -- a token in a value, a secret-named field, nested", () => {
    ...
    new Audit(s.audit, quiet).record(ACTOR, "token.created", "t1", {
      new: { name: "ci", note: "use gw_0123456789abcdefghij to deploy", password: "hunter2hunter2", nested: { authorization: "Bearer abcdefghijklmnopqrstuvwxyz" } },
    });
    ...
    expect(all(s.audit)[0]!.new).toMatchObject({ name: "ci", password: "[redacted]", nested: { authorization: "[redacted]" } });
  });

  test("a field NAMED `key` is swallowed by redaction: settings changes must say `setting`", () => {
    const s = setup();
    const audit = new Audit(s.audit, quiet);
    audit.record(ACTOR, "user.updated", null, { new: { key: "surfaces.ui", setting: "surfaces.ui" } });
  ...
  test("nobody authenticated (a failed login) is `system` with no id; the target says who it was about", () => {
    const s = setup();
    new Audit(s.audit, quiet).record(null, "auth.login.failed", "ada@example.com", { new: { ip: "203.0.113.7" } });
    expect(all(s.audit)[0]).toMatchObject({ actorType: "system", actorId: null, target: "ada@example.com", new: { ip: "203.0.113.7" } });
  });

  test("never throws: the action already happened, so a failed write is logged, not raised", () => {
    const lines: string[] = [];
    const broken = { append: () => { throw new Error("disk full"); } } as unknown as AuditRepo;
...
describe("GET /v1/audit", () => {
  ...
  test("pages newest-first; nextBefore walks to the end; limit is capped", async () => {
    ...
    const first = await (await get("/audit?limit=2")).json() as { entries: { target: string }[]; nextBefore: number | null };
    expect(first.entries.map((e) => e.target)).toEqual(["u5", "u4"]);
    const rest = await (await get(`/audit?limit=10&before=${first.nextBefore}`)).json() as typeof first;
    ...
    expect((await (await get("/audit?action=preview.deploy")).json() as typeof first).entries).toEqual([]);
  });
});
