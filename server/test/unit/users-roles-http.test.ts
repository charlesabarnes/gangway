...
async function make() {
  ...
  const app = createApp({
    ...
    v1: (api) => {
      userRoutes(api, s.accounts); roleRoutes(api, s.roles); tokenRoutes(api, tokens);
      api.delete("/previews/:id", requirePermission("previews.destroy"), (c) => c.json({ destroyed: c.req.param("id") }));
    },
  ...
  const login = async (email: string, password = PASSWORD) =>
    (await call("/v1/auth/login", { method: "POST", json: { email, password } })).headers.get("set-cookie")!.split(";")[0]!;

  await s.admin();
  const ada = await login("ada@example.com");
  const addUser = async (email: string, roleId: string) =>
    ((await (await call("/v1/users", { method: "POST", as: ada, json: { email, password: PASSWORD, roleId } })).json()) as { user: { id: string } }).user;
...
describe("/v1/users", () => {
  test("an admin creates, lists, re-roles and disables; no response ever carries password material", async () => {
    const t = await make();
    const made = await t.call("/v1/users", { method: "POST", as: t.ada, json: { email: "Bob@Example.com", password: PASSWORD, roleId: "member" } });
    expect(made.status).toBe(201);
    const { user } = await made.json() as { user: { id: string; email: string; roleId: string; disabled: boolean } };
    ...
    for (const leak of ["scrypt", "password", "salt", "hash"]) expect(listed.toLowerCase()).not.toContain(leak);

    expect(await (await t.call(`/v1/users/${user.id}`, { method: "PATCH", as: t.ada, json: { roleId: "viewer", disabled: true } })).json())
  ...
  test("members and viewers may not read or manage accounts", async () => {
    ...
    expect((await t.call("/v1/users", { method: "POST", as: bob, json: { email: "x@example.com", password: PASSWORD, roleId: "admin" } })).status).toBe(403);
  });
  ...
  test("validation: unknown role 422, duplicate email 409, empty patch 422, unknown user 404, extra fields 422", async () => {
    ...
    expect((await post({ email: "c@example.com", password: PASSWORD, roleId: "wizard" })).status).toBe(422);
    expect((await post({ email: "bob@example.com", password: PASSWORD, roleId: "member" })).status).toBe(409);
    expect((await post({ email: "c@example.com", password: "short", roleId: "member" })).status).toBe(422);
    expect((await post({ email: "c@example.com", password: PASSWORD, roleId: "member", admin: true })).status).toBe(422);
    expect((await t.call(`/v1/users/${bob.id}`, { method: "PATCH", as: t.ada, json: {} })).status).toBe(422);
    expect((await t.call("/v1/users/nobody", { method: "PATCH", as: t.ada, json: { disabled: true } })).status).toBe(404);
  });

  test("the last enabled admin cannot lock the door behind them: 409", async () => {
    const t = await make();
    const me = (await (await t.call("/v1/users", { as: t.ada })).json() as { users: { id: string }[] }).users[0]!;
    expect((await t.call(`/v1/users/${me.id}`, { method: "PATCH", as: t.ada, json: { roleId: "viewer" } })).status).toBe(409);
    expect((await t.call(`/v1/users/${me.id}`, { method: "PATCH", as: t.ada, json: { disabled: true } })).status).toBe(409);
  });
  ...
  test("a password reset logs the target out everywhere", async () => {
    ...
    expect((await t.call(`/v1/users/${bob.id}`, { method: "PATCH", as: t.ada, json: { password: "an entirely new password" } })).status).toBe(200);
    expect((await t.call("/v1/tokens", { as: session })).status).toBe(401);
    expect((await t.call("/v1/tokens", { as: await t.login("bob@example.com", "an entirely new password") })).status).toBe(200);
  });
});

describe("/v1/roles", () => {
  type RolesBody = { roles: { id: string; builtin: boolean; editable: boolean; permissions: string[] }[]; catalogue: { id: string; feature: string }[] };
  ...
  test("GET shows the three seeded roles, what each grants, and everything that CAN be granted", async () => {
    ...
    expect(body.roles.map((r) => [r.id, r.builtin, r.editable])).toEqual([["admin", true, false], ["member", true, true], ["viewer", true, true]]);
  ...
  test("an edit reaches a member's OPEN session and their TOKEN on the very next request -- no re-login", async () => {
    ...
    await t.addUser("bob@example.com", "member");
    const session = await t.login("bob@example.com");
    const { secret } = await (await t.call("/v1/tokens", { method: "POST", as: session, json: { name: "ci", scopes: ["deploy"] } })).json() as { secret: string };
    ...
    const put = await t.call("/v1/roles/member/permissions", { method: "PUT", as: t.ada, json: { permissions: without } });
    expect(put.status).toBe(200);
    expect((await put.json() as { role: { permissions: string[] } }).role.permissions).toEqual([...without].sort());
    ...
    expect((await (await t.call("/v1/auth/session", { as: session })).json() as { permissions: string[] }).permissions).not.toContain("previews.destroy");

    await t.call("/v1/roles/member/permissions", { method: "PUT", as: t.ada, json: { permissions: DEFAULT_ROLE_PERMISSIONS.member } });
    expect((await t.call("/v1/previews/p1", { method: "DELETE", as: session })).status).toBe(200);
  });

  test("admin is 409, an unknown role 404, an unknown permission 422 -- and nothing was changed by any of them", async () => {
    const t = await make();
    const put = (role: string, permissions: unknown) => t.call(`/v1/roles/${role}/permissions`, { method: "PUT", as: t.ada, json: { permissions } });
    ...
    expect([...t.s.roles.for("viewer")].sort()).toEqual([...DEFAULT_ROLE_PERMISSIONS.viewer].sort());
  ...
  test("a role can be emptied; its people can still log in, and can do nothing", async () => {
    ...
    expect((await t.call("/v1/roles/viewer/permissions", { method: "PUT", as: t.ada, json: { permissions: [] } })).status).toBe(200);
    const vic = await t.login("vic@example.com");
    expect(await (await t.call("/v1/auth/session", { as: vic })).json()).toMatchObject({ authenticated: true, permissions: [] });
  });
  ...
  test("only roles.manage may edit, and the edit is audited with the whole before and after", async () => {
    ...
    expect((await t.call("/v1/roles/member/permissions", { method: "PUT", as: bob, json: { permissions: [...ALL_PERMISSIONS] } })).status).toBe(403);

    await t.call("/v1/roles/viewer/permissions", { method: "PUT", as: t.ada, json: { permissions: ["previews.read"] } });
    expect(t.s.auditRepo.page({ limit: 1 }).entries[0]).toMatchObject({
      actorType: "user", action: "role.permissions.changed", target: "viewer",
      old: { permissions: [...DEFAULT_ROLE_PERMISSIONS.viewer].sort() }, new: { permissions: ["previews.read"] },
    });
  });

  test("the env admin token can edit roles too: the matrix is recoverable with no account at all", async () => {
    const t = await make();
    expect((await t.call("/v1/roles/viewer/permissions", { method: "PUT", as: ENV_TOKEN, json: { permissions: ["previews.read"] } })).status).toBe(200);
    expect(t.s.auditRepo.page({ limit: 1 }).entries[0]).toMatchObject({ actorType: "token", actorId: "env:admin" });
  });
});
