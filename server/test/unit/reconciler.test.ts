...
function setup(o: { orphans?: "stop" | "report"; hangUp?: boolean } = {}) {
  ...
  const clients = {
    for: () => ({
      hostId: "local",
      info: async () => { if (daemon.down) throw new Error("connect ECONNREFUSED 127.0.0.1:23750"); return daemon.info; },
      listContainers: async (opts: ListOptions = {}) => { daemon.lists.push(opts); await Bun.sleep(5); return daemon.containers; },
      stopContainer: async (id: string) => { daemon.stopped.push(id); },
    }),
  };
  const compose: ComposeRunner = {
    async *stream(argv, _h, opt): AsyncGenerator<ComposeEvent> {
      daemon.composed.push("up");
      if (o.hangUp) await new Promise<void>((r) => opt.signal?.addEventListener("abort", () => r()));
  ...
  const reconciler = new Reconciler({
    ctx, routes, clients, logger: new Logger("info", {}, (l) => lines.push(l)), env: {}, ...(o.orphans ? { orphans: o.orphans } : {}),
  });

  /** A container exactly as the deploy pipeline would have labelled it. */
  const containerFor = (route: Route, project: string, over: Partial<ContainerSummary> & { instance?: string; hostPort?: number } = {}): ContainerSummary => ({
    id: `c-${route.hostname}`, names: [`${project}-${route.service}-1`], image: "traefik/whoami:v1.10",
    state: "running", status: "Up", createdAt: new Date(),
    labels: containerLabels(route, { instance: over.instance ?? "default", env: "test", project, hostId: "local", visibility: "public" }),
    ports: [{ ip: "127.0.0.1", containerPort: route.containerPort, hostPort: over.hostPort ?? route.upstream.port, protocol: "tcp" }],
    ...over,
  });

  const deployed = async (name: string) => {
    const res = await deploy(ctx, { actor: ACTOR, name, visibility: "public", source: { kind: "image", image: "traefik/whoami:v1.10", port: 80 } });
  ...
  return { ctx, reconciler, daemon, routes, previews, table, hosts, containerFor, deployed, eventTypes, lines };
}
...
describe("a quiet pass", () => {
  test("route and container agree: nothing changes, nothing is published, the host is ready", async () => {
    ...
    expect(report.hosts).toEqual([{ hostId: "local", reachable: true, error: null, containers: 1 }]);
...
describe("rule 1: provably ours, or not at all", () => {
  test("the daemon is asked only for THIS instance and env", async () => {
    ...
    expect(s.daemon.lists[0]).toEqual({ all: true, filters: { label: ["gangway.managed=true", "gangway.instance=default", "gangway.env=test"] } });
  });
  ...
  test("another installation's container is never stopped, even if the daemon hands it to us, even with garbage labels", async () => {
    ...
    const foreign = s.containerFor({ ...route, hostname: "theirs.preview.localhost" }, preview.project, { id: "c-foreign", instance: "someone-else" });
    delete foreign.labels["gangway.service"]; // incomplete: would be a textbook StopOrphan
    const anonymous = s.containerFor({ ...route, hostname: "anon.preview.localhost" }, preview.project, { id: "c-anon" });
  ...
  test("OUR container with incomplete labels is an orphan holding a port: stopped (§11)", async () => {
    const s = setup();
    const { route, preview } = await s.deployed("hello");
    const orphan = s.containerFor({ ...route, hostname: "orphan.preview.localhost", previewId: "01J0000000000000000000000Z" }, preview.project, { id: "c-orphan", hostPort: 31050 });
  ...
  test("orphans=report names what it would stop, and stops nothing", async () => {
    ...
    const { route, preview } = await s.deployed("hello");
    const orphan = s.containerFor({ ...route, hostname: "orphan.preview.localhost" }, preview.project, { id: "c-orphan" });
...
describe("rule 2: an unreachable host is not an empty host", () => {
  test("the tunnel drops: nothing is decided. It comes back: the host is ready again", async () => {
    ...
    expect(s.hosts.get("local")).toMatchObject({ state: "unreachable", lastError: expect.stringContaining("ECONNREFUSED") });
  ...
  test("the WRONG daemon is an error, not a blip: never listed, never touched", async () => {
    ...
    expect(report.changes).toEqual([]);
    expect(s.previews.get(preview.id)!.state).toBe("awake");
    expect(s.hosts.get("local")).toMatchObject({ state: "error", lastError: expect.stringContaining("Docker Desktop") });
  });
});
...
describe("rule 3: work in flight is untouchable", () => {
  test("a deploy in `starting` has a route and no container yet. That is not a discrepancy", async () => {
    ...
    const res = await deploy(s.ctx, { actor: ACTOR, name: "slow", visibility: "public", source: { kind: "image", image: "x", port: 80 } });
    await Bun.sleep(20);
...
describe("§11's table", () => {
  ...
  test("no route, container running: the route is rebuilt from labels", async () => {
    ...
    const report = await s.reconciler.run();
    expect(report.changes).toEqual(["hello.preview.localhost: route rebuilt from container labels"]);
    expect(s.table.lookup(route.hostname)).toMatchObject({ previewId: preview.id, upstreamPort: route.upstream.port, state: "awake" });
...
describe("interrupted by a restart", () => {
  const interrupted = async (s: ReturnType<typeof setup>, state: "building" | "starting" | "destroying") => {
  ...
  test("starting, container up but NOT answering: failed, and the stack is released", async () => {
    ...
    s.daemon.probe = false;
    await s.reconciler.run();
    expect(s.previews.get(preview.id)).toMatchObject({ state: "failed", error: "interrupted by a server restart while starting" });
...
describe("scan mapping", () => {
  test("labels are read field by field; garbage becomes undefined, never a throw", () => {
    expect(scanLabels({ "gangway.preview_id": "p", "gangway.container_port": "80", "gangway.primary": "yes", "gangway.visibility": "secret", "gangway.version": "2" }))
      .toEqual({ previewId: "p", hostname: undefined, service: undefined, containerPort: 80, visibility: undefined, primary: undefined, version: 2 });
    expect(scanLabels({ "gangway.container_port": "80; rm -rf /", "gangway.hostname": "" })).toMatchObject({ containerPort: undefined, hostname: undefined });
  });

  test("the published port is the one bound to the LABELLED container port, tcp only", () => {
    const host = { id: "local", upstream: { dial: "direct" as const, address: "10.0.0.5", proxy: null } };
    const c: ContainerSummary = {
      id: "c", names: ["n"], image: "i", state: "paused", status: "", createdAt: new Date(),
    ...
    expect(toScanned(c, host)).toMatchObject({ publishedPort: 31004, upstreamHost: "10.0.0.5", state: "running" });
    expect(toScanned({ ...c, state: "created", ports: [] }, host)).toMatchObject({ publishedPort: null, state: "exited" });
  });
});
