...
const repository = {
  name: "web-app", full_name: "acme/web-app", private: false, fork: false,
  owner: { login: "acme" }, clone_url: "https://github.com/acme/web-app.git",
};
const pull = (over: Record<string, unknown> = {}) => ({
  number: 123, title: "Add the thing", draft: false, merged: false, html_url: "https://github.com/acme/web-app/pull/123",
  user: { login: "dev" },
  head: { sha: "0123456789abcdef0123456789abcdef01234567", ref: "feature/thing", repo: { full_name: "acme/web-app" } },
  base: { ref: "main", repo: { full_name: "acme/web-app" } },
  ...over,
});
const prEvent = (action: string, over: Record<string, unknown> = {}) => ({
  action, repository, installation: { id: 4242 }, pull_request: pull(over),
});
const commentEvent = (body: string, association = "COLLABORATOR", over: Record<string, unknown> = {}) => ({
  action: "created", repository, installation: { id: 4242 },
  issue: { number: 123, pull_request: { url: "https://api.github.com/repos/acme/web-app/pulls/123" } },
...
describe("parsing GitHub's webhook", () => {
  test("pull_request opened / synchronize / reopened / ready_for_review are one event", () => {
    for (const action of ["opened", "synchronize", "reopened", "ready_for_review"] as const) {
      ...
      expect(ev.pr).toMatchObject({
        number: 123, headSha: "0123456789abcdef0123456789abcdef01234567", headRef: "feature/thing", baseRef: "main",
        fromFork: false, draft: false, author: "dev",
        repo: { forge: "github", fullName: "acme/web-app", owner: "acme", name: "web-app", installationId: "4242", cloneUrl: "https://github.com/acme/web-app.git" },
      });
    }
  });

  test("closed carries whether it merged", () => {
    expect(parseGitHubEvent("pull_request", prEvent("closed", { merged: true }))).toMatchObject({ type: "pr.closed", merged: true });
    expect(parseGitHubEvent("pull_request", prEvent("closed"))).toMatchObject({ type: "pr.closed", merged: false });
  });

  test("a head in another repository is a fork; a DELETED head repository counts as one too", () => {
    const fork = parseGitHubEvent("pull_request", prEvent("opened", { head: { sha: "a".repeat(40), ref: "x", repo: { full_name: "stranger/web-app" } } }));
    expect(fork).toMatchObject({ type: "pr.updated", pr: { fromFork: true } });
    const gone = parseGitHubEvent("pull_request", prEvent("opened", { head: { sha: "a".repeat(40), ref: "x", repo: null } }));
  ...
  test("actions we do not act on, other events, and pings are `ignored` with a reason -- never a throw", () => {
    expect(parseGitHubEvent("pull_request", prEvent("labeled"))).toEqual({ type: "ignored", reason: "pull_request.labeled" });
    expect(parseGitHubEvent("ping", { zen: "Keep it logically awesome." })).toEqual({ type: "ignored", reason: "ping" });
    ...
    expect(parseGitHubEvent("pull_request", { action: "opened" })).toMatchObject({ type: "ignored" });
    expect(parseGitHubEvent("pull_request", { action: "opened", repository, pull_request: pull() })).toEqual({ type: "ignored", reason: "pull_request without an installation" });
  });
  ...
  test("/preview <verb> as the first line of a PR comment is a command, with who said it", () => {
    ...
    expect(ev).toEqual({
      type: "pr.command", command: "deploy", number: 123, commentId: 987, author: "maintainer", association: "collaborator",
      repo: { forge: "github", fullName: "acme/web-app", owner: "acme", name: "web-app", installationId: "4242", cloneUrl: "https://github.com/acme/web-app.git", private: false },
    });
    expect(parseGitHubEvent("issue_comment", commentEvent("/preview status", "OWNER"))).toMatchObject({ association: "owner" });
    expect(parseGitHubEvent("issue_comment", commentEvent("/preview status", "MEMBER"))).toMatchObject({ association: "member" });
    // Having contributed is not authority over the Docker host.
    expect(parseGitHubEvent("issue_comment", commentEvent("/preview status", "CONTRIBUTOR"))).toMatchObject({ association: "other" });
    expect(parseGitHubEvent("issue_comment", commentEvent("/preview status", "NONE"))).toMatchObject({ association: "other" });
  });

  test("a comment on an issue, an edited comment, or prose mentioning /preview is not a command", () => {
    expect(parseGitHubEvent("issue_comment", commentEvent("/preview deploy", "OWNER", { issue: { number: 5 } }))).toMatchObject({ type: "ignored", reason: "a comment on an issue, not a pull request" });
    expect(parseGitHubEvent("issue_comment", commentEvent("/preview deploy", "OWNER", { action: "edited" }))).toMatchObject({ type: "ignored" });
    expect(parseGitHubEvent("issue_comment", commentEvent("you can run /preview deploy to see it"))).toEqual({ type: "ignored", reason: "not a /preview command" });
  });
});
...
describe("the signature", () => {
  ...
  test("round-trips, and the verify is over the RAW bytes", () => {
    ...
    expect(verifySignature(secret, enc(JSON.stringify({ action: "opened" }, null, 2)), sig)).toBe(false);
  });
...
function fakeGitHub(o: { tokenTtlMs?: number; failEdit?: boolean } = {}) {
  ...
  const fetchImpl: FetchLike = async (url, init) => {
    ...
    if (method === "POST" && /^\/app\/installations\/\d+\/access_tokens$/.test(path)) {
      ...
      tokenSeq += 1;
      return json(201, { token: `ghs_${tokenSeq}`, expires_at: new Date(now + (o.tokenTtlMs ?? 3_600_000)).toISOString() });
    }
    const presented = headers.get("authorization") ?? "";
    if (!presented.startsWith("token ghs_") || revoked.has(presented.slice("token ".length))) return json(401, { message: "Bad credentials" });

    if (method === "GET" && path === "/repos/acme/web-app/pulls/123") return json(200, pull());
    if (method === "GET" && path === "/repos/acme/web-app/pulls/999") return json(404, { message: "Not Found" });
    if (method === "POST" && path === "/repos/acme/web-app/issues/123/comments") {
      const id = nextId++; comments.set(id, body.body); return json(201, { id });
    }
    ...
    if (method === "PATCH" && edit) {
      ...
      comments.set(id, body.body); return json(200, { id });
    }
    if (method === "POST" && path === "/repos/acme/web-app/deployments") return json(201, { id: 555 });
    if (method === "POST" && path === "/repos/acme/web-app/deployments/555/statuses") return json(201, { id: 777 });
    return json(404, { message: `unhandled ${method} ${path}` });
  };

  return { fetchImpl, calls, comments, revoke: (t: string) => revoked.add(t), clock: { now: () => now, advance: (ms: number) => { now += ms; } } };
}

function appAndForge(o: Parameters<typeof fakeGitHub>[0] = {}) {
  const gh = fakeGitHub(o);
  const app = new GitHubApp({ credentials: () => ({ appId: "12345", privateKey: PEM }), fetch: gh.fetchImpl, baseUrl: BASE, log: silent(), now: gh.clock.now });
  const forge = new GitHubForge({ app, webhookSecret: () => "s3cret" });
  const repo = { forge: "github" as const, fullName: "acme/web-app", owner: "acme", name: "web-app", cloneUrl: "https://github.com/acme/web-app.git", installationId: "4242", private: false };
...
describe("GitHubApp", () => {
  ...
  test("an unconfigured App is a clear 422, not a crypto error", () => {
    const app = new GitHubApp({ credentials: () => ({ appId: "", privateKey: "" }), fetch: async () => new Response(""), log: silent() });
  ...
  test("concurrent first calls mint ONCE", async () => {
    const { app, gh } = appAndForge();
    const tokens = await Promise.all([app.installationToken("4242"), app.installationToken("4242"), app.installationToken("4242")]);
  ...
  test("a 401 on an installation call forgets the cached token, so the next call mints a fresh one", async () => {
    const { app, gh } = appAndForge();
    expect((await app.asInstallation("4242", "GET", "/repos/acme/web-app/pulls/123")).status).toBe(200);
    gh.revoke("ghs_1");
    expect((await app.asInstallation("4242", "GET", "/repos/acme/web-app/pulls/123")).status).toBe(401);
...
describe("GitHubForge", () => {
  test("verify: signature over the raw body, and the delivery id comes back", () => {
    ...
    const headers = new Headers({ "x-hub-signature-256": signPayload("s3cret", raw), "x-github-delivery": "d-1", "x-github-event": "pull_request" });
  ...
  test("verify refuses everything while no secret is configured", () => {
    ...
    const raw = enc("{}");
    expect(forge.verify(new Headers({ "x-hub-signature-256": signPayload("", raw), "x-github-delivery": "d" }), raw)).toEqual({ ok: false, reason: "no webhook secret is configured" });
  });
  ...
  test("pullRequest fetches as the installation; 404 is notFound", async () => {
    ...
    expect(pr).toMatchObject({ number: 123, headSha: "0123456789abcdef0123456789abcdef01234567", fromFork: false });
    expect(gh.calls.at(-1)).toMatchObject({ method: "GET", path: "/repos/acme/web-app/pulls/123", auth: "token ghs_1" });
  ...
  test("deployments: created against the head sha as a transient environment, then given a status", async () => {
    ...
    expect(id).toBe(555);
    expect(gh.calls.at(-1)!.body).toMatchObject({ ref: "abc", environment: "preview/web-app-pr-123", transient_environment: true, production_environment: false, auto_merge: false, required_contexts: [] });
    await forge.setDeploymentStatus(repo, 555, "success", { environmentUrl: "https://web-app-pr-123.preview.example.com/" });
