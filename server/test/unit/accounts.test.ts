...
describe("login", () => {
  ...
  test("unknown email, wrong password and disabled account are ONE error, and each costs a real verify", async () => {
    const s = setup();
    await s.admin();
    const bob = await s.accounts.createUser((s.sessions.resolve((await s.accounts.login("ada@example.com", PASSWORD, META)).secret))!.actor, { email: "bob@example.com", password: PASSWORD, roleId: "member" });
    await s.accounts.updateUser(s.sessions.resolve((await s.accounts.login("ada@example.com", PASSWORD, META)).secret)!.actor, bob.id, { disabled: true });

    const calls: string[] = [];
    const { verify, verifyDummy } = s.passwords;
    s.passwords.verify = function (...a) { calls.push("verify"); return verify.apply(this, a); };
    s.passwords.verifyDummy = function (...a) { calls.push("dummy"); return verifyDummy.apply(this, a); };

    const errors = [];
    for (const [email, pw] of [["nobody@example.com", PASSWORD], ["ada@example.com", "not the password!"], ["bob@example.com", PASSWORD]] as const) {
      errors.push(await s.accounts.login(email, pw, META).catch((e) => ({ code: e.code, status: e.status, message: e.message })));
    }
    expect(new Set(errors.map((e) => JSON.stringify(e))).size).toBe(1);
    expect(errors[0]).toEqual({ code: "unauthorized", status: 401, message: "wrong email or password" });
  ...
  test("5 failures lock the account: 429 with Retry-After, the RIGHT password is refused too, and scrypt is not run", async () => {
    const s = setup();
    await s.admin();
    for (let i = 0; i < 5; i++) await expect(s.accounts.login("ada@example.com", "wrong wrong wrong", META)).rejects.toMatchObject({ status: 401 });

    let ran = 0;
    const { verify } = s.passwords;
    s.passwords.verify = function (...a) { ran++; return verify.apply(this, a); };
    await expect(s.accounts.login("ada@example.com", PASSWORD, META)).rejects.toMatchObject({ status: 429, headers: { "retry-after": "60" } });
    expect(ran).toBe(0);

    s.clock.t += MIN;
    expect((await s.accounts.login("ada@example.com", PASSWORD, META)).user.email).toBe("ada@example.com");
  });

  test("a spray of blocked attempts writes ONE audit row, not one per attempt", async () => {
    const s = setup();
    await s.admin();
    for (let i = 0; i < 5; i++) await s.accounts.login("ada@example.com", "wrong wrong wrong", META).catch(() => {});
    for (let i = 0; i < 50; i++) await s.accounts.login("ada@example.com", "wrong wrong wrong", META).catch(() => {});
...
describe("first-run setup", () => {
  test("racing requests make exactly one admin; the loser sees a 404, as if setup were never there", async () => {
    const s = setup();
    const results = await Promise.allSettled([1, 2, 3].map((i) => s.accounts.setupFirstAdmin(`admin${i}@example.com`, PASSWORD, META)));
    expect(results.filter((r) => r.status === "fulfilled")).toHaveLength(1);
    for (const r of results) if (r.status === "rejected") expect(r.reason).toMatchObject({ status: 404 });
  ...
  test("Bootstrap is pending exactly while there are no users", async () => {
    ...
    expect(url).toMatch(/^https:\/\/app\.preview\.localhost:8443\/setup\?token=gw_setup_[A-Za-z0-9_-]{43}$/);
...
describe("administering users", () => {
  ...
  test("create: the role must exist and the email must be free", async () => {
    ...
    await expect(s.accounts.createUser(s.actor, { email: "bob@example.com", password: PASSWORD, roleId: "wizard" })).rejects.toMatchObject({ status: 422 });
    await s.accounts.createUser(s.actor, { email: "bob@example.com", password: PASSWORD, roleId: "viewer" });
    await expect(s.accounts.createUser(s.actor, { email: "bob@example.com", password: PASSWORD, roleId: "viewer" })).rejects.toMatchObject({ status: 409 });
  });

  test("the last enabled admin cannot be demoted or disabled -- not even by themselves", async () => {
    const s = await withAdmin();
    await expect(s.accounts.updateUser(s.actor, s.ada.id, { roleId: "member" })).rejects.toMatchObject({ status: 409 });
    await expect(s.accounts.updateUser(s.actor, s.ada.id, { disabled: true })).rejects.toMatchObject({ status: 409 });

    const bob = await s.accounts.createUser(s.actor, { email: "bob@example.com", password: PASSWORD, roleId: "admin" });
    ...
    expect((await s.accounts.updateUser(s.actor, s.ada.id, { roleId: "member" })).roleId).toBe("member");
  });

  test("a role change applies to a session that is ALREADY open, on its next request", async () => {
    const s = await withAdmin();
    const bob = await s.accounts.createUser(s.actor, { email: "bob@example.com", password: PASSWORD, roleId: "member" });
  ...
  test("disabling ends every session at once; so does a password reset, and the new password works", async () => {
    ...
    await expect(s.accounts.login("bob@example.com", PASSWORD, META)).rejects.toMatchObject({ status: 401 });

    const second = (await s.accounts.login("bob@example.com", "an entirely new password", META)).secret;
    await s.accounts.updateUser(s.actor, bob.id, { disabled: true });
  ...
  test("the audit trail says who changed what, with before and after", async () => {
    const s = await withAdmin();
    const bob = await s.accounts.createUser(s.actor, { email: "bob@example.com", password: PASSWORD, roleId: "member" });
    await s.accounts.updateUser(s.actor, bob.id, { roleId: "viewer", password: "an entirely new password" });
    const entry = s.auditRepo.page({ limit: 1 }).entries[0]!;
    expect(entry).toMatchObject({
      actorType: "user", actorId: s.ada.id, action: "user.updated", target: bob.id,
      old: { roleId: "member", disabled: false }, new: { roleId: "viewer", disabled: false, passwordReset: true },
    });
...
describe("changing your own password", () => {
  test("needs the current one; ends every OTHER session; keeps the one in hand", async () => {
    ...
    await expect(s.accounts.changeOwnPassword(actor, "not my password!", "a brand new password", META)).rejects.toMatchObject({ status: 403 });
    ...
    expect((await s.accounts.login("ada@example.com", "a brand new password", META)).user.email).toBe("ada@example.com");
  });

  test("it is a password oracle for whoever holds a session, so it shares login's lockout", async () => {
    const s = setup();
    await s.admin();
    const actor = s.sessions.resolve((await s.accounts.login("ada@example.com", PASSWORD, META)).secret)!.actor;
    for (let i = 0; i < 5; i++) await s.accounts.changeOwnPassword(actor, `guess number ${i} here`, "a brand new password", META).catch(() => {});
    await expect(s.accounts.changeOwnPassword(actor, PASSWORD, "a brand new password", META)).rejects.toMatchObject({ status: 429 });
  });

  test("a token is not a person", async () => {
    const s = setup();
    const { tokenActor } = await import("../../src/auth/actor.ts");
    await expect(s.accounts.changeOwnPassword(tokenActor("env:admin", ["admin"]), "x", "a brand new password", META)).rejects.toMatchObject({ status: 403 });
  });
});
...
describe("session lifetime", () => {
  test("a malformed cookie is refused without touching the database", async () => {
    ...
    s.sessionsRepo.findActive = function (...a) { reads++; return findActive.apply(this, a); };
    for (const junk of ["", "short", "x".repeat(10_000), "has spaces in it and is forty-three chars!!", "../../etc/passwd"]) expect(s.sessions.resolve(junk)).toBeNull();
    expect(reads).toBe(0);
  });
  ...
  test("use keeps it alive; 7 idle days end it; 30 days end it however busy", async () => {
    const s = setup();
    const { secret } = await s.admin();
    for (let day = 0; day < 29; day++) { s.clock.t += DAY; expect(s.sessions.resolve(secret)).not.toBeNull(); }
    s.clock.t += DAY + 1;
  ...
  test("the sliding write is throttled: a busy session writes once per window, not once per request", async () => {
    ...
    s.sessionsRepo.touch = function (...a) { const wrote = touch.apply(this, a); if (wrote) writes++; return wrote; };
    for (let i = 0; i < 100; i++) { s.clock.t += 1000; s.sessions.resolve(secret); }
