...
describe("client metadata documents: what may be fetched", () => {
  ...
  test("the document must name itself, list redirects, and be a public client", () => {
    ...
    expect(() => parseDocument(CLAUDE, ok({ client_id: CLAUDE, redirect_uris: [] }))).toThrow(
      "redirect_uris",
    );
    expect(() =>
      parseDocument(
...
describe("authorize: nothing is redirected until the redirect is trusted", () => {
  ...
  test("a repeated parameter is refused; offline_access is accepted and ignored; no scope means both", async () => {
    ...
    expect(s.oauth.view(s.ada, (off as { requestId: string }).requestId).requested).toEqual([
      "deploy",
    ]);
    const none = await s.oauth.authorize(s.authorizeQuery({ scope: null }));
    expect(s.oauth.view(s.ada, (none as { requestId: string }).requestId).requested).toEqual([
      "read",
      "deploy",
    ]);
  });
...
async function http() {
  ...
  const toolCall = (token: string, name: string, args: unknown) =>
    call(mcpH, "mcp.preview.localhost:8443", "/", {
      method: "POST",
      headers: {
...
describe("the flow over HTTP, as claude.ai drives it", () => {
  test("401 -> resource metadata -> AS metadata -> authorize -> /connect -> approve -> token -> a tool call", async () => {
    ...
    const denied = await h.call(h.mcpH, "mcp.preview.localhost:8443", "/", {
      method: "POST",
      body: "{}",
    });
    expect(denied.status).toBe(401);
    expect(denied.headers.get("www-authenticate")).toBe(
      `Bearer resource_metadata="${RESOURCE}/.well-known/oauth-protected-resource", scope="read deploy"`,
    );
    const prm = await (
      await h.call(h.mcpH, "mcp.preview.localhost:8443", "/.well-known/oauth-protected-resource")
    ).json();
  ...
  test("a read-only grant calling deploy gets the step-up 403: insufficient_scope, asking for deploy", async () => {
    ...
    expect(res.headers.get("www-authenticate")).toBe(
      `Bearer resource_metadata="${RESOURCE}/.well-known/oauth-protected-resource", error="insufficient_scope", scope="read deploy"`,
    );
  ...
  test("ADR-0021: a deploy grant rebuilds its own preview; someone else's steps up to update, and an update grant may", async () => {
    ...
    expect(theirs.headers.get("www-authenticate")).toBe(
      `Bearer resource_metadata="${RESOURCE}/.well-known/oauth-protected-resource", error="insufficient_scope", scope="read deploy update"`,
    );
  ...
  test("MCP off: every OAuth endpoint is a 404; the UI off: no resource metadata, and a 401 says only Bearer", async () => {
    ...
    expect(
      (
        await h.call(h.mcpH, "mcp.preview.localhost:8443", "/", { method: "POST", body: "{}" })
