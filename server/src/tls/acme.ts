...
export type AcmeApi = Pick<acme.Client,
  "createAccount" | "getAccountUrl" | "createOrder" | "getAuthorizations" | "getChallengeKeyAuthorization"
  | "completeChallenge" | "waitForValidStatus" | "finalizeOrder" | "getCertificate">;

export type AcmeConnect = (o: { directoryUrl: string; accountKey: string; accountUrl?: string }) => AcmeApi;
...
export class AcmeProvider implements CertProvider {
  ...
  load(domains: string[]): CertBundle | null {
    ...
    if (!row || row.source !== this.#o.directoryUrl || !row.notAfter || row.notAfter.getTime() <= this.#now()) return null;
    let covered: string[];
    try {
    ...
    return {
      materials: [{
        serverName: domains[0]!, key: row.keyPem, cert: row.certPem + (row.chainPem ?? ""),
        notBefore: row.notBefore ?? undefined, notAfter: row.notAfter, issuer: row.issuer ?? undefined,
      }],
    };
  }
  ...
  async #client(): Promise<AcmeApi> {
    ...
    if (known) return this.#connect({ directoryUrl, accountKey: known.keyPem, accountUrl: known.url });
    ...
    store.set(ACCOUNTS_KEY, { ...accounts, [directoryUrl]: { keyPem, url: client.getAccountUrl() } });
  ...
  async issue(domains: string[], signal?: AbortSignal): Promise<CertBundle> {
    ...
    try {
      ...
      const pending: { authz: acme.Authorization; challenge: (typeof authzs)[number]["challenges"][number] }[] = [];
      const byName = new Map<string, string[]>();
      for (const authz of authzs) {
        if (authz.status === "valid") continue; // the CA remembers a recent validation
        const challenge = authz.challenges.find((c) => c.type === "dns-01");
        if (!challenge) throw new Error(`the CA offered no dns-01 challenge for ${authz.identifier.value}`);
      ...
      for (const [name, values] of byName) {
        ...
        if (!(await dns.waitForPropagation(name, values))) {
          throw new Error(`TXT records at ${name} did not propagate; the order was abandoned before the CA was asked to validate`);
        }
      }
      ...
      for (const { authz, challenge } of pending) {
        ...
        logger.debug("acme authorization valid", { identifier: authz.identifier.value, wildcard: authz.wildcard === true });
      }

      const [key, csr] = await acme.crypto.createCsr({ commonName: domains[0]!, altNames: domains }, await acme.crypto.createPrivateEcdsaKey());
      ...
      certs.put({
        domain: domains[0]!, certPem: leaf, keyPem: key.toString(), chainPem: chainPem || null,
        issuer: info.issuer.commonName, source: directoryUrl, notBefore: info.notBefore, notAfter: info.notAfter,
      });
      logger.info("acme certificate issued", { domains, issuer: info.issuer.commonName, notAfter: info.notAfter.toISOString(), ms: this.#now() - began });
      return {
        materials: [{
          serverName: domains[0]!, key: key.toString(), cert: leaf + chainPem,
          notBefore: info.notBefore, notAfter: info.notAfter, issuer: info.issuer.commonName,
        }],
      };
    } finally {
      // Step 3.
      for (const r of created) {
        await dns.removeTxt(r.recordId, r.name).catch((e) => logger.warn("could not remove an acme TXT record", { name: r.name, err: e }));
      }
    }
  }
}
