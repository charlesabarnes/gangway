...
export function checkFiles(files: Record<string, string>): { count: number; bytes: number } {
  ...
  if (paths.length === 0) throw unprocessable("files is empty: name at least one file, e.g. index.html");
  ...
  if (bytes > MAX_BYTES) throw unprocessable(`at most ${MAX_BYTES / 1024 / 1024} MiB of file contents`);
 ...
 */
export async function packFiles(files: Record<string, string>): Promise<{ archive: Uint8Array; digest: string }> {
  checkFiles(files);
  ...
  const ended = new Promise<void>((res, rej) => { p.on("end", res); p.on("error", rej); });
  for (const path of paths) {
    const body = Buffer.from(files[path]!, "utf8");
    hash.update(`${path}\0${body.length}\0`).update(body);
    await new Promise<void>((res, rej) => p.entry({ name: path, size: body.length, mode: 0o644, mtime: new Date(0) }, body, (e) => (e ? rej(e) : res())));
  }
  p.finalize();
  await ended;
  return { archive: new Uint8Array(gzipSync(Buffer.concat(chunks))), digest: `sha256:${hash.digest("hex")}` };
}
