...
export function refusalDetail(detail: Record<string, unknown> | undefined): string {
  if (!detail) return "";
  const lines: string[] = [];
  for (const r of Array.isArray(detail["reasons"]) ? detail["reasons"] as { level?: string; found?: string; then?: string }[] : []) {
    if (r.found && r.then) lines.push(`  ${r.level && r.level !== "info" ? `${r.level}: ` : ""}${r.found} -> ${r.then}`);
  }
  for (const i of Array.isArray(detail["issues"]) ? detail["issues"] as { path?: string; message?: string }[] : []) {
    if (i.message) lines.push(`  gangway.yml ${i.path ?? ""}: ${i.message}`);
  }
  for (const v of Array.isArray(detail["violations"]) ? detail["violations"] as unknown[] : []) lines.push(`  ${typeof v === "string" ? v : JSON.stringify(v)}`);
  if (typeof detail["compose"] === "string") lines.push(`  compose: ${detail["compose"].slice(-500)}`);
  return lines.length ? `\n${lines.slice(0, 20).join("\n")}` : "";
}

/** How an upload was read (ADR-0016), in the words the New screen uses. */
export function describePlan(plan: AppPlan): string {
  const what = plan.kind === "own" ? "the upload's own compose file or Dockerfile"
    : `${plan.runtime}${plan.version ? ` ${plan.version}` : ""}`;
  const runs = plan.kind === "own" ? null
    : plan.start ? `runs \`${cmdText(plan.start)}\``
    : plan.entry ? `runs ${plan.entry}`
    : plan.serve.kind === "static" ? "the files are served by nginx" : null;
  const addons = plan.addons.length ? `add-ons: ${plan.addons.map((a) => `${a.id} ${a.version}`).join(", ")}` : null;
  const reasons = plan.reasons.slice(0, PLAN_REASONS_SHOWN).map((r) => `  ${r.level === "info" ? "" : `${r.level}: `}${r.found} -> ${r.then}`);
  const more = plan.reasons.length > PLAN_REASONS_SHOWN ? [`  … ${plan.reasons.length - PLAN_REASONS_SHOWN} more in logs`] : [];
...
const DeployArgs = z.object({
  ...
  upload: z.string().max(64).optional().describe("Instead of files, for anything bigger than a page or two: \"new\" returns a one-use URL to PUT a .tar.gz of the app to (with curl) -- nothing is deployed yet. Then call deploy again with upload: \"<the id>\" and the usual options. Faster than files, and deploys exactly the bytes on disk."),
  preview: z.string().optional().describe("Rebuild THIS existing preview (its name, URL or id) at the same URL. files are then changes: only the files named are written. With upload: the whole source is replaced by the upload."),
  ...
  passwordLogin: z.enum(["inherit", "on", "off", "only"]).optional().describe("Who can open it. off: anyone with the password; on: people signed in to gangway, or anyone with the password; only: only people signed in to gangway (no password); inherit follows the server."),
  ...
  check: z.array(z.string().regex(CHECK_PATH, "a path starting with /, no spaces")).max(MAX_CHECKS).optional()
    .describe("Paths to GET once it answers, e.g. [\"/\", \"/api/health\"]. Each one's status comes back with the result, so you need not curl them."),
});
...
export class Tools {
  ...
  server(scope: CallScope): McpServer {
    ...
    s.registerTool("logs", {
      title: "Preview logs",
      description: "A preview's logs: the pipeline (build, start, gangway's own lines) and the runtime (what its containers print, e.g. your server's startup line and its errors). Read this when a deploy failed or the app misbehaves.",
      inputSchema: z.object({
        preview: PreviewRef,
        lines: z.number().int().min(1).max(500).optional().describe("How many per section, default 80."),
        source: z.enum(LOG_SOURCES).optional().describe("pipeline, runtime, or all (the default)."),
        service: z.string().max(63).optional().describe("Runtime lines of one service only, e.g. web or postgres."),
      }),
      annotations: { readOnlyHint: true },
    }, (args) => this.#guard("logs", () => this.logs(scope, args.preview, args.lines ?? 80, { source: args.source, service: args.service })));
  ...
  missingFor(actor: Actor, tool: string, args: unknown): Permission | null {
    ...
    try { owner = this.#d.ctx.previews.ownerOf(resolvePreview(this.#d.ctx, ref).id); } catch { return null; }
  ...
  async #guard(tool: ToolName, run: () => Promise<string>): Promise<CallToolResult> {
    try {
      ...
      if (err instanceof AppError) return failure(`${tool} refused: ${err.message}${refusalDetail(err.detail)}`);
  ...
  async deploy(scope: CallScope, args: DeployArgs, progress: (n: number, message: string) => void = () => {}): Promise<string> {
    ...
    const given = [args.files !== undefined, args.upload !== undefined, args.image !== undefined, args.git !== undefined].filter(Boolean).length;
    ...
    if (args.upload !== undefined) {
      ...
      source = { kind: "tarball", archive: taken.archive, digest: taken.digest, runtime: "auto", ...(args.port === undefined ? {} : { port: args.port }), ...(addons === undefined ? {} : { addons }) };
    } else if (args.files) {
  ...
  async #redeploy(scope: CallScope, args: DeployArgs, wait: number, addons: ReturnType<typeof addonQuery.parse> | undefined): Promise<string> {
    ...
    if (args.image !== undefined || args.git !== undefined) throw unprocessable("preview rebuilds from files or an upload; an image or a repository is a new deploy");
    if (args.upload !== undefined && (args.files !== undefined || args.remove !== undefined)) throw unprocessable("upload replaces the whole source; files and remove edit it -- give one or the other");
    const target = resolvePreview(ctx, args.preview!);
    if (!mayRebuild(scope.actor, ctx.previews.ownerOf(target.id))) {
      throw new MissingPermission(REDEPLOY_PERMISSION, `${nameOf(ctx, target)} was deployed by someone else, and "previews.update_own" covers only your own. Deploy the change as a new preview instead, or ask for the update scope`);
    }
    ...
    if (args.upload !== undefined) {
      ...
      if (Object.keys(files).length === 0 && addons === undefined) throw unprocessable("nothing to change: give files, remove, upload or addons");
      change = { kind: "edit", files };
    }
    const res = await redeploy(ctx, { actor: scope.actor, previewId: target.id, change, ...(addons === undefined ? {} : { addons }) })
      .finally(() => taken?.done());
   ...
   */
  async #report(p: Preview, plan: AppPlan | undefined, check: readonly string[] | undefined): Promise<string> {
    const { ctx } = this.#d;
    const out: string[] = [];
    ...
    if (ctx.sources && p.source.kind === "tarball" && (await ctx.sources.has(p.id))) {
      ...
      if (m.files.length <= MANIFEST_SHOWN) {
        out.push(`files as deployed (sha256, first 12 hex; compare with shasum -a 256):\n${m.files.map((f) => `  ${f.sha256.slice(0, 12)}  ${String(f.bytes).padStart(8)}  ${f.path}`).join("\n")}`);
      } else {
        out.push(`files as deployed: ${m.files.length}${m.truncated ? "+" : ""} (too many to list; the preview page shows them)`);
      }
    }
    if (check && check.length > 0) {
      ...
      if (route && host) {
        ...
        const target = { hostname: route.hostname, upstream: { host: route.upstreamHost, port: route.upstreamPort } };
        const got = await Promise.all(check.map(async (path) => `${path} ${(await probe(target, host, path)) ?? "no answer"}`));
  ...
  #uploads(): Uploads {
    if (!this.#d.uploads) throw unprocessable("uploads are not available on this server; send files instead");
    return this.#d.uploads;
  }

  #take(id: string, actor: Actor): Taken {
    if (id === "new") throw unprocessable('upload: "new" asks for an upload URL; deploy from it with the id it returns');
  ...
  #issueUpload(scope: CallScope, args: DeployArgs): string {
    const others = (["files", "image", "git", "remove"] as const).filter((k) => args[k] !== undefined);
    if (others.length > 0) throw unprocessable(`upload: "new" only asks for a URL; ${others.join(", ")} go with the deploy that follows`);
  ...
  async logs(scope: CallScope, ref: string, lines: number, opts: { source?: LogSource | undefined; service?: string | undefined } = {}): Promise<string> {
    const { ctx } = this.#d;
    ...
    if (source !== "runtime") parts.push(`pipeline (build, start, gangway):\n${logTail(ctx, p.id, n)}`);
    if (source !== "pipeline") {
      const rt = await runtimeLogs(ctx, p, { tail: n, service: opts.service });
      const body = rt.lines === null ? `(${rt.why})` : rt.lines.length === 0 ? "(nothing printed yet)" : rt.lines.join("\n");
      parts.push(`runtime (what the containers print${opts.service ? `, ${opts.service} only` : ""}):\n${body}`);
    }
    return parts.join("\n\n");
  }
