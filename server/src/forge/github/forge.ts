...
export class GitHubForge implements Forge {
  ...
  async pullRequest(repo: ForgeRepo, number: number): Promise<PullRequest> {
    const r = await this.#app.asInstallation<Parameters<typeof pullRequestOf>[0]>(
      repo.installationId,
      "GET",
  ...
  async createDeployment(
    pr: Pick<PullRequest, "repo" | "headSha">,
    environment: string,
  ): Promise<number> {
    const r = await this.#app.asInstallation<{ id?: number; message?: string }>(
      pr.repo.installationId,
      "POST",
      `${repoPath(pr.repo)}/deployments`,
      {
        ref: pr.headSha,
        environment,
        auto_merge: false,
        ...
        description: "gangway preview",
      },
    );
    if (r.status !== 201 || typeof r.body?.id !== "number") {
      throw internal(
        `GitHub answered ${r.status} creating a deployment: ${r.body?.message ?? ""}`.trim(),
        { status: r.status },
      );
    }
    return r.body.id;
