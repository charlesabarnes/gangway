...
import { parsePreviewCommand, type Association, type ForgeEvent, type ForgeRepo, type PullRequest } from "../forge.ts";
...
type GhRepo = {
  full_name?: string; name?: string; owner?: GhUser; clone_url?: string; private?: boolean; fork?: boolean;
};
type GhPullRequest = {
  number?: number; title?: string; draft?: boolean; merged?: boolean; html_url?: string; user?: GhUser;
...
function repoOf(r: GhRepo | undefined, installationId: string): ForgeRepo | null {
  if (!r || typeof r.full_name !== "string" || typeof r.name !== "string" || typeof r.clone_url !== "string") return null;
  const owner = r.owner?.login ?? r.full_name.split("/")[0];
  if (typeof owner !== "string" || owner === "") return null;
  return { forge: "github", fullName: r.full_name, owner, name: r.name, cloneUrl: r.clone_url, installationId, private: r.private === true };
}
...
export function pullRequestOf(p: GhPullRequest | undefined, repo: ForgeRepo): PullRequest | null {
  if (!p || typeof p.number !== "number" || typeof p.head?.sha !== "string" || typeof p.head.ref !== "string") return null;
  const headRepo = p.head.repo?.full_name;
  return {
    repo, number: p.number, title: p.title ?? "", headSha: p.head.sha, headRef: p.head.ref, baseRef: p.base?.ref ?? "",
    ...
    draft: p.draft === true, author: p.user?.login ?? "", htmlUrl: p.html_url ?? "",
  };
}
...
export function parseGitHubEvent(event: string | null, payload: unknown): ForgeEvent {
  ...
  switch (event) {
    case "pull_request": {
      ...
      if (installationId === "") return { type: "ignored", reason: "pull_request without an installation" };
      const pr = pullRequestOf(p.pull_request, repo);
      if (!pr) return { type: "ignored", reason: "pull_request without a head" };
      const action = p.action ?? "";
      if (action === "closed") return { type: "pr.closed", pr, merged: p.pull_request?.merged === true };
    ...
    case "issue_comment": {
      ...
      if (!p.issue?.pull_request || typeof p.issue.number !== "number") return { type: "ignored", reason: "a comment on an issue, not a pull request" };
    ...
    case "ping": return { type: "ignored", reason: "ping" };
    default: return { type: "ignored", reason: `event ${event ?? "(none)"}` };
  }
}
