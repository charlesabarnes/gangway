/**
 * Runtimes: an upload with no Dockerfile becomes an image. What to do is
...
/**
 * The plan for an upload on disk: the same function the New screen calls through
 * `/v1/runtimes/plan`.
 */
...
/**
 * workerd: esbuild bundles the entry; every `.wasm` import becomes a wasm module; secrets are
 * bound by name.
 */
  ...
  switch (id) {
    case "static":
      files["nginx.conf"] = nginxConf(
        listen,
        plan.serve.kind === "static" ? plan.serve.fallback : "spa",
      );
      return {
        ...
        note:
          plan.serve.kind === "static" && plan.serve.fallback === "spa"
            ? "index.html for unknown paths (single-page app fallback)"
            : plan.serve.kind === "static" && plan.serve.fallback === "404"
              ? "404.html for unknown paths"
              : "no index.html: directories are listed",
      };
    case "php": {
...
export async function writeRuntime(
  ...
  // gangway.yml < the project's secrets < the add-ons' own addresses.
...
/**
 * gangway.yml's stack policy, plus its release command, as the generated compose file's
 * `x-gangway`.
 */
