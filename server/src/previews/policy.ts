...
export type PolicyInput = { source: DeploySource; actor: Actor; template?: string | undefined; projectId?: string | undefined };
...
export const triggerOf = (source: DeploySource, actor: Actor): Trigger =>
  source.kind === "pr" || source.kind === "pushed" ? "pr" : actor.kind === "user" ? "manual" : "api";
...
export class PolicyResolver implements Policy {
  ...
  resolve(input: PolicyInput): ResolvedPolicy {
    ...
    if (input.projectId !== undefined) {
      ...
      if (!project) throw new AppError("unprocessable", `no such project: ${input.projectId}`, { project: input.projectId });
    } else {
    ...
    if (!this.#warned.has(wanted)) {
      ...
      this.#d.logger?.warn("template not found; deploying with the default template", { template: wanted, trigger, project: project?.slug });
    }
...
export function fixedPolicy(fields: Partial<Omit<Template, "id" | "createdAt" | "updatedAt">> = {}): Policy {
  ...
  return {
    resolve: (input) => ({ template, project: undefined, trigger: triggerOf(input.source, input.actor) }),
    default: () => template,
  };
}
