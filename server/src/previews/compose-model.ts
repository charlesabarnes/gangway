...
function parseExtension<T>(schema: z.ZodType<T>, raw: unknown, where: string): T {
  ...
  throw unprocessable(`${where}: x-gangway${i?.path.length ? `.${i.path.join(".")}` : ""}: ${i?.message ?? "invalid"}`);
}
...
function policyViolations(project: string, doc: Json, sourceDirs: readonly string[]): string[] {
  ...
  for (const [name, raw] of Object.entries(obj(doc["services"]))) {
    ...
    if (s["container_name"] !== undefined) out.push(`${at}: container_name is not allowed (it defeats per-preview namespacing)`);
  ...
  for (const kind of ["networks", "volumes"] as const) {
    for (const [key, raw] of Object.entries(obj(doc[kind]))) {
      ...
      if (r["external"] !== undefined && r["external"] !== false) out.push(`${at}: external is not allowed`);
      if (typeof r["name"] === "string" && r["name"] !== `${project}_${key}`) out.push(`${at}: a custom name is not allowed`);
      if (kind === "volumes" && Object.keys(obj(r["driver_opts"])).length > 0) out.push(`${at}: driver_opts are not allowed (a bind mount in disguise)`);
      if (kind === "networks" && r["driver"] !== undefined && r["driver"] !== "bridge") out.push(`${at}: only the bridge driver is allowed`);
    }
  }
  return out;
}
...
export function parseComposeModel(project: string, resolved: unknown, sourceDirs: readonly string[] = []): ComposeModel {
  ...
  const services = Object.entries(rawServices).map(([name, raw]): ServiceModel => {
    ...
    return {
      ...
      publishedTargets: arr(s["ports"]).map(obj)
        .filter((p) => (p["protocol"] ?? "tcp") === "tcp" && typeof p["target"] === "number")
        .map((p) => p["target"] as number),
      exposed: arr(s["expose"]).map((e) => Number(String(e).split("/")[0])).filter((n) => Number.isInteger(n) && n > 0),
...
export type ExposedService = { service: string; containerPort: number; subdomain: string | null; primary: boolean };
...
export function selectExposed(model: ComposeModel): ExposedService[] {
  ...
  if (chosen.length === 0) {
    const publishing = model.services.filter((s) => s.publishedTargets.length > 0 && s.x.expose !== false);
  ...
  if (primaries.length > 1) throw unprocessable(`only one service may be primary; found ${primaries.map((s) => s.name).join(", ")}`);
  ...
  return chosen.map((s) => {
    ...
    return { service: s.name, containerPort, subdomain: s.x.subdomain ?? null, primary: s.x.primary === true };
  });
}
...
export function planRoutes(i: PlanInput): PlannedRoute[] {
  ...
  const routes = i.exposed.map((e, idx): PlannedRoute => {
    const built = buildLabel({ kind: "slug", slug: i.slug }, {
      service: e.subdomain ?? e.service, isPrimary: e.primary, isSingleService: i.exposed.length === 1,
    });
    if (!built.ok) throw unprocessable(`cannot build a hostname for service "${e.service}": ${built.message}`, { reason: built.reason });
    return {
  ...
  for (const r of routes) {
    if (seen.has(r.hostname)) throw unprocessable(`two services resolve to the same hostname ${r.hostname}; give one a distinct \`x-gangway.subdomain\``);
    seen.add(r.hostname);
  }
  return routes;
}
...
const envKey = (service: string) => `GANGWAY_URL_${service.toUpperCase().replace(/[^A-Z0-9]+/g, "_")}`;
...
function asMap(v: unknown): Record<string, unknown> {
  ...
  return Object.fromEntries(v.map((e) => {
    const text = String(e);
...
export function buildStack(i: StackInput): string {
  ...
  const ownership = {
    [LABEL.instance]: i.ctx.instance, [LABEL.env]: i.ctx.env,
    [LABEL.project]: i.ctx.project, [LABEL.previewId]: previewId,
  };
  const urls = Object.fromEntries(i.routes.map((r) => [envKey(r.service), publicOriginFor(r.hostname, i.origin)]));
  ...
  for (const [name, raw] of Object.entries(services)) {
    ...
    const mine = route ? buildLabels(labelsFromRoute({ ...route, createdAt: i.createdAt }, i.ctx)) : ownership;
    ...
    const theirs = Object.fromEntries(Object.entries(asMap(svc["labels"])).filter(([k]) => !k.startsWith("gangway.")));
    ...
    if (route) {
      svc["ports"] = [{ mode: "ingress", host_ip: i.publishBind, target: route.containerPort, published: String(route.upstream.port), protocol: "tcp" }];
    } else {
      delete svc["ports"];
    }
