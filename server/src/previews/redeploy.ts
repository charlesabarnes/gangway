...
async function applyEdits(srcDir: string, files: SourceEdits): Promise<number> {
  ...
  for (const [rel, text] of Object.entries(files)) {
    ...
    for (let i = 1; i <= parts.length; i++) {
      ...
      if (st?.isSymbolicLink()) throw unprocessable(`cannot write ${JSON.stringify(rel)}: ${parts.slice(0, i).join("/")} is a symlink`);
      if (st && i < parts.length && !st.isDirectory()) throw unprocessable(`cannot write ${JSON.stringify(rel)}: ${parts.slice(0, i).join("/")} is a file`);
      if (st && i === parts.length && st.isDirectory()) throw unprocessable(`cannot write ${JSON.stringify(rel)}: it is a directory`);
    }
    if (text === null) {
...
const routesOf = (ctx: PreviewContext, previewId: string): PlannedRoute[] =>
  ctx.table.forPreview(previewId).map((e) => ({
    hostname: e.hostname, previewId: e.previewId, service: e.service, containerPort: e.containerPort,
    upstream: { host: e.upstreamHost, port: e.upstreamPort }, primary: e.primary,
  }));
...
export async function redeploy(ctx: PreviewContext, input: RedeployInput): Promise<RedeployResult> {
  ...
  if (!["awake", "asleep", "failed"].includes(preview.state)) {
    throw conflict(`the preview is ${preview.state}; wait for it to settle`, { state: preview.state });
  }
  if (ctx.inflight.has(id) || ctx.teardowns.has(id)) throw conflict("a deploy of this preview is still running");
  const abort = new AbortController();
  let settle!: (o: RedeployOutcome) => void;
  const done = new Promise<RedeployOutcome>((r) => { settle = r; });
  ...
  try {
    ...
    if (input.change.kind === "replace") {
      ...
      ctx.logs.append(id, "system", `rebuilding from a new upload (requested by ${actorId(input.actor)}): ${r.files} files, ${r.totalBytes} bytes`);
    } else {
    ...
    if (want.size !== got.size || [...want].some((k) => !got.has(k))) {
      throw unprocessable("the new source exposes different services or ports than this preview; deploy it as a new preview instead", {
        expected: [...want].sort(), got: [...got].sort(),
      });
    }
    ...
    settle({ preview: ctx.previews.get(id) ?? preview, buildId, outcome: "failed", error: e instanceof Error ? e.message : String(e) });
    throw e;
  }
  ...
  ctx.audit?.record(input.actor, "preview.redeploy", id, {
    new: { project: preview.project, change: input.change.kind, runtime: plan.runtime.kind === "tarball" ? plan.runtime.runtime ?? "own" : null, buildId },
  });
...
async function run(ctx: PreviewContext, r: RunInput): Promise<RedeployOutcome> {
  ...
  const outcome = (o: "succeeded" | "failed", error?: string): RedeployOutcome => {
    ...
    return { preview: ctx.previews.get(id) ?? preview, buildId, outcome: o, ...(error ? { error } : {}) };
  };
  ...
  try {
    await writeFile(stackPath, buildStack({
      resolved: r.resolved, planProject: PLAN_PROJECT, model: r.model, routes: r.routes, createdAt: preview.createdAt,
      ctx: { instance: ctx.instance, env: ctx.env, project: preview.project, hostId: host.id, visibility: preview.visibility },
    ...
    if (r.signal.aborted) return { preview: ctx.previews.get(id) ?? preview, buildId, outcome: "failed", error: "cancelled" };
    const message = redactString(e instanceof Error ? e.message : String(e));
    if (!(e instanceof StepFailed)) ctx.logger.error("redeploy pipeline error", { previewId: id, err: e });
...
type Base = { project: string; files: string[]; projectDirectory: string; docker: string | undefined };

/** The image ids the project's containers run now. Empty on any failure: cleanup is best-effort. */
async function imageIds(ctx: PreviewContext, host: Host, base: Base, cwd: string): Promise<Set<string>> {
  try {
    const res = await ctx.compose.capture(composeArgv({ ...base, command: "images", args: ["--quiet"] }), host, { cwd });
    return new Set(res.code === 0 ? res.stdout.split("\n").map((l) => l.trim()).filter((l) => /^(sha256:)?[0-9a-f]{12,64}$/.test(l)) : []);
  } catch {
    return new Set();
  }
}
 ...
 */
async function removeReplaced(ctx: PreviewContext, host: Host, base: Base, cwd: string, before: Set<string>, previewId: string): Promise<void> {
  ...
  try {
    for (const img of before) {
      ...
      const res = await ctx.compose.capture([docker, "image", "inspect", "--format", "{{len .RepoTags}} {{len .RepoDigests}}", img], host, { cwd: empty });
      if (res.code !== 0 || res.stdout.trim() !== "0 0") continue;
      const removed = await ctx.compose.capture([docker, "image", "rm", img], host, { cwd: empty });
      if (removed.code === 0) ctx.logs.append(previewId, "system", `removed the replaced image ${img.replace(/^sha256:/, "").slice(0, 12)}`);
    }
  } catch (e) {
