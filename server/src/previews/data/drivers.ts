...
export type QueryResult = { columns: string[]; rows: Cell[][]; truncated: boolean; message: string | null };
...
export function queryArgv(addon: AddonId, text: string, write: boolean): string[] {
  switch (addon) {
    case "postgres":
      ...
      return ["psql", "-X", "-q", "-U", ADDON_USER, "-d", ADDON_USER, "--csv", "-v", "ON_ERROR_STOP=1",
        "-c", `SET statement_timeout = '${STATEMENT_TIMEOUT_S}s'`,
...
export function rowsQuery(addon: Exclude<AddonId, "redis">, t: Table, limit: number, offset: number): string {
  const q = addon === "postgres" ? (s: string) => `"${s.replace(/"/g, '""')}"` : (s: string) => `\`${s.replace(/`/g, "``")}\``;
...
export function tokenize(text: string): string[] {
  ...
  for (let i = 0; i < text.length; i++) {
    ...
    if (quote) {
      ...
      if (c === "\\" && quote === '"' && i + 1 < text.length) { cur += text[++i]!; continue; }
      cur += c;
...
const REDIS_READ = new Set([
  "GET", "MGET", "STRLEN", "GETRANGE", "EXISTS", "TYPE", "TTL", "PTTL", "KEYS", "SCAN", "DBSIZE", "INFO", "PING", "RANDOMKEY",
  "HGET", "HMGET", "HGETALL", "HKEYS", "HVALS", "HLEN", "HEXISTS", "HSCAN",
  "LRANGE", "LLEN", "LINDEX", "SMEMBERS", "SCARD", "SISMEMBER", "SSCAN", "SRANDMEMBER",
  "ZRANGE", "ZRANGEBYSCORE", "ZREVRANGE", "ZCARD", "ZSCORE", "ZRANK", "ZSCAN", "ZCOUNT",
  "XRANGE", "XREVRANGE", "XLEN", "XINFO", "OBJECT", "MEMORY", "TIME", "DUMP",
]);
/** Never, write switch or not: they block, stream forever, or reach past this one database. */
const REDIS_NEVER = new Set(["MONITOR", "SUBSCRIBE", "PSUBSCRIBE", "SSUBSCRIBE", "SYNC", "PSYNC", "SHUTDOWN", "CONFIG", "DEBUG", "MODULE", "ACL", "REPLICAOF", "SLAVEOF", "CLIENT", "FAILOVER", "CLUSTER", "SAVE", "BGSAVE", "BGREWRITEAOF", "MIGRATE", "BLPOP", "BRPOP", "BLMOVE", "BZPOPMIN", "BZPOPMAX", "XREAD", "XREADGROUP", "WAIT", "SCRIPT", "FUNCTION"]);
...
export function redisRefusal(text: string, write: boolean): string | null {
  let argv: string[];
  try { argv = redisArgv(text); } catch (e) { return e instanceof Error ? e.message : "cannot parse the command"; }
...
export function parseCsv(text: string): { columns: string[]; rows: Cell[][] } {
  const records: Cell[][] = [];
  let rec: Cell[] = [], field = "", quoted = false, inQuotes = false, any = false;
  const endField = () => { rec.push(!quoted && field === "" ? null : field); field = ""; quoted = false; };
  for (let i = 0; i < text.length; i++) {
    const c = text[i]!;
    any = true;
    if (inQuotes) {
    ...
    } else if (c === '"') { inQuotes = true; quoted = true; }
    else if (c === ",") endField();
    else if (c === "\n") { endField(); records.push(rec); rec = []; any = false; }
    else if (c !== "\r") field += c;
  }
  if (any || field !== "" || rec.length > 0) { endField(); records.push(rec); }
...
export function parseBatch(text: string): { columns: string[]; rows: Cell[][] } {
  const unescape = (s: string): Cell => (s === "NULL" ? null : s.replace(/\\(.)/g, (_, c: string) => (c === "t" ? "\t" : c === "n" ? "\n" : c === "0" ? "\0" : c)));
...
export function parse(addon: AddonId, text: string): { columns: string[]; rows: Cell[][] } {
  return addon === "postgres" ? parseCsv(text) : addon === "mysql" ? parseBatch(text) : parseRedis(text);
}
