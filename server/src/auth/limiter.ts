...
export type LimiterVerdict = { ok: true } | { ok: false; retryAfterSec: number; reason: "ip" | "email" };
...
export class LoginLimiter {
  ...
  constructor(o: LimiterOptions = {}, now: () => number = Date.now) {
    this.#o = {
      ipMax: o.ipMax ?? 10, ipWindowMs: o.ipWindowMs ?? 15 * MIN,
      emailFree: o.emailFree ?? 5, emailBaseLockMs: o.emailBaseLockMs ?? MIN, emailMaxLockMs: o.emailMaxLockMs ?? 15 * MIN,
      forgetAfterMs: o.forgetAfterMs ?? 60 * MIN, maxKeys: o.maxKeys ?? 10_000,
    };
    this.#now = now;
  ...
  check(ip: string, email: string): LimiterVerdict {
    ...
    if (e && e.lockedUntil > now) return { ok: false, retryAfterSec: sec(e.lockedUntil), reason: "email" };

    const recent = this.#recent(ip, now);
    if (recent.length >= this.#o.ipMax) return { ok: false, retryAfterSec: sec(recent[0]! + this.#o.ipWindowMs), reason: "ip" };
    return { ok: true };
  }
  ...
  fail(ip: string, email: string): void {
    ...
    const failures = (prev && prev.lastFailure > now - this.#o.forgetAfterMs ? prev.failures : 0) + 1;
    const over = failures - this.#o.emailFree;
    const lock = over < 0 ? 0 : Math.min(this.#o.emailMaxLockMs, this.#o.emailBaseLockMs * 2 ** over);
