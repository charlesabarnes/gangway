...
import { can, ENV_ADMIN_TOKEN_ID, permissionsForScopes, type Actor, type TokenVerifier } from "./actor.ts";
...
export class Tokens {
  ...
  constructor(repo: TokensRepo, roles: RolePermissions, audit: AuditSink, now: () => number = Date.now) {
    this.#repo = repo;
    this.#roles = roles;
    this.#audit = audit;
    this.#now = now;
  ...
  readonly verify: TokenVerifier = (presented) => {
    ...
    if (!owner) return { kind: "token", tokenId: token.id, scopes: token.scopes, permissions: bundle };
    const role = this.#roles.for(owner.roleId);
    const permissions = new Set<Permission>([...bundle].filter((p) => role.has(p)));
    return { kind: "token", tokenId: token.id, scopes: token.scopes, permissions, userId: owner.id };
  };
  ...
  mint(actor: Actor, input: { name: string; scopes: readonly Scope[]; expiresIn?: string | undefined }): { token: ApiToken; secret: string } {
    ...
    for (const scope of scopes) {
      ...
      if (missing.length > 0) throw unprocessable(`your role does not cover the "${scope}" scope`, { scope, missing });
    }

    let expiresAt: number | null = null;
    if (input.expiresIn !== undefined) {
      const ms = parseDuration(input.expiresIn);
      if (ms === null || ms <= 0) throw unprocessable(`expiresIn ${JSON.stringify(input.expiresIn)} is not a duration like 12h or 90d`);
      expiresAt = this.#now() + ms;
    }

    const secret = `gw_${randomBytes(32).toString("base64url")}`;
    const token = this.#repo.create({
      id: ulid(this.#now()), name: input.name, prefix: secret.slice(0, PREFIX_LEN), tokenHash: hashOf(secret), scopes, userId: owner, expiresAt,
    });
    this.#audit.record(actor, "token.created", token.id, { new: { name: token.name, scopes, userId: owner, expiresAt: token.expiresAt } });
    return { token, secret };
  }
  ...
  list(actor: Actor, o: { all?: boolean } = {}): ApiToken[] {
    ...
    return actor.kind === "user" ? this.#repo.listForUser(actor.userId) : can(actor, "tokens.manage_all") ? this.#repo.listAll() : [];
  }
  ...
  revoke(actor: Actor, id: string): ApiToken {
    ...
    if (!token || !(mine || can(actor, "tokens.manage_all"))) throw notFound(`no such token: ${id}`);
    if (this.#repo.revoke(id, this.#now())) {
      this.#audit.record(actor, "token.revoked", id, { old: { name: token.name, scopes: token.scopes, userId: token.userId } });
    }
