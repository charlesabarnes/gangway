...
export function workflowFor(project: Pick<Project, "name" | "slug">, apiOrigin: string, port = 3000): string {
  return TEMPLATE.replaceAll("__NAME__", project.name).replaceAll("__API__", apiOrigin).replaceAll("__SLUG__", project.slug).replaceAll("__PORT__", String(port));
}
