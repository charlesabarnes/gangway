...
export function settingsRoutes(
  api: Hono<AppEnv>, settings: Settings, audit: AuditSink, templates?: Pick<TemplatesRepo, "get">,
  ...
  api.put("/settings", requirePermission("settings.write"), async (c) => {
    ...
    for (const [key, raw] of Object.entries(values)) {
      ...
      if (key.startsWith("previews.password.")) throw conflict(`"${key}" is changed through PUT /v1/settings/preview-password`, { key });
  ...
  api.put("/settings/preview-password", requirePermission("settings.write"), async (c) => {
    ...
    for (const d of [SETTINGS.previewPasswordMode, SETTINGS.previewPasswordShared, ...(login === undefined ? [] : [SETTINGS.previewPasswordLogin])]) {
      if (settings.isManagedByConfig(d.key)) throw conflict(`"${d.key}" is managed by config and cannot be changed at runtime`, { key: d.key });
    }
    ...
    if (mode === "shared" && value === undefined && had === null) throw unprocessable("a shared password needs a value the first time", { key: SETTINGS.previewPasswordShared.key });
    if (value !== undefined && mode !== "shared") throw unprocessable("a value only goes with mode \"shared\"");
    if (value !== undefined) {
      if (!hashPassword) throw unprocessable("password-protected previews are not available on this server");
    ...
    audit.record(c.get("actor"), "settings.changed", null, {
      old: { [SETTINGS.previewPasswordLogin.key]: oldLogin, [SETTINGS.previewPasswordMode.key]: old, [SETTINGS.previewPasswordShared.key]: had === null ? "[unset]" : "[set]" },
      new: { [SETTINGS.previewPasswordLogin.key]: login ?? oldLogin, [SETTINGS.previewPasswordMode.key]: mode, [SETTINGS.previewPasswordShared.key]: value !== undefined ? "[changed]" : had === null ? "[unset]" : "[set]" },
    });
    return c.json({ settings: settings.view() });
  });
}
