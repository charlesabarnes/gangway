...
export type CreateToken = {
  id: string; name: string; prefix: string; tokenHash: string; scopes: readonly Scope[];
  userId: string | null; expiresAt: number | null;
};
...
export class TokensRepo {
  ...
  create(t: CreateToken): ApiToken {
    this.#db.run(
      ...
      { id: t.id, name: t.name, prefix: t.prefix, hash: t.tokenHash, scopes: JSON.stringify(t.scopes), user: t.userId, exp: t.expiresAt, now: this.#now() },
    );
    return this.get(t.id)!;
  }

  get(id: string): ApiToken | undefined {
    const r = this.#db.get<TokenRow>(`SELECT ${TOKEN_COLUMNS} FROM api_tokens WHERE id = $id`, { id });
   ...
   */
  findActiveByHash(tokenHash: string, now: number = this.#now()): { token: ApiToken; owner: User | null } | undefined {
    const r = this.#db.get<TokenRow & { u_id: string | null; u_email: string | null; u_role_id: string | null; u_disabled: number | null; u_created_at: number | null }>(
      `SELECT ${TOKEN_COLUMNS.split(", ").map((c) => `t.${c}`).join(", ")},
    ...
    const owner: UserRow | null = r.u_id === null ? null
      : { id: r.u_id, email: r.u_email!, role_id: r.u_role_id!, disabled: r.u_disabled!, created_at: r.u_created_at! };
  ...
  listForUser(userId: string): ApiToken[] {
    return this.#db.query<TokenRow>(`SELECT ${TOKEN_COLUMNS} FROM api_tokens WHERE user_id = $u ORDER BY created_at DESC, id`, { u: userId }).map(rowToToken);
  }

  listAll(): ApiToken[] {
    return this.#db.query<TokenRow>(`SELECT ${TOKEN_COLUMNS} FROM api_tokens ORDER BY created_at DESC, id`).map(rowToToken);
  }
  ...
  revoke(id: string, now: number = this.#now()): boolean {
    return this.#db.run("UPDATE api_tokens SET revoked_at = $now WHERE id = $id AND revoked_at IS NULL", { id, now }).changes > 0;
  }
