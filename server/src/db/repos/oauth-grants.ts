...
export class OAuthGrantsRepo {
    ...
    return (
      this.#db.run(
        ...
        WHERE id = $id AND refresh_hash = $from AND revoked_at IS NULL`,
        {
          ...
          from,
          ah: next.accessHash,
          aexp: next.accessExpiresAt,
          rh: next.refreshHash,
          rexp: next.refreshExpiresAt,
          now: this.#now(),
        },
      ).changes > 0
    );
  ...
  touch(id: string, staleBefore: number, now: number = this.#now()): void {
    this.#db.run(
      "UPDATE oauth_grants SET last_used_at = $now WHERE id = $id AND (last_used_at IS NULL OR last_used_at < $stale)",
  ...
  revoke(id: string, now: number = this.#now()): boolean {
    return (
      this.#db.run(
        "UPDATE oauth_grants SET revoked_at = $now WHERE id = $id AND revoked_at IS NULL",
        { id, now },
      ).changes > 0
    );
  }
  ...
  revokeAllFor(userId: string, now: number = this.#now()): number {
    return this.#db.run(
      "UPDATE oauth_grants SET revoked_at = $now WHERE user_id = $u AND revoked_at IS NULL",
      { u: userId, now },
    ).changes;
  }
