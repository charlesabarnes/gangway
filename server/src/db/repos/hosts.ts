...
export class HostsRepo {
  ...
  upsert(h: HostInput): Host {
    this.#db.run(
      ...
      {
        id: h.id, name: h.name, docker_host: h.dockerHost, expect_name: h.expectName,
        capabilities: JSON.stringify(h.capabilities), publish_bind: h.publishBind,
        upstream_dial: h.upstream.dial, upstream_address: h.upstream.address,
        upstream_proxy: h.upstream.proxy,
        port_range_start: h.ports.rangeStart, port_range_end: h.ports.rangeEnd,
        state: h.state ?? "unknown", created_at: this.#now(),
      },
    );
