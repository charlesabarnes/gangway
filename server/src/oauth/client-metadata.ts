...
export function checkClientIdUrl(raw: string): URL {
  let u: URL;
  try { u = new URL(raw); } catch { throw new ClientMetadataError("client_id is not a URL"); }
  if (u.protocol !== "https:") throw new ClientMetadataError("client_id must be an https URL");
  if (u.port !== "" && u.port !== "443") throw new ClientMetadataError("client_id must use the default https port");
  if (u.username || u.password) throw new ClientMetadataError("client_id must not carry credentials");
  if (u.hash) throw new ClientMetadataError("client_id must not have a fragment");
  if (u.pathname === "/" || u.pathname === "") throw new ClientMetadataError("client_id must have a path");
  if (u.href !== raw) throw new ClientMetadataError("client_id must be a normalized URL");
  if (isIP(u.hostname.replace(/^\[|\]$/g, "")) !== 0) throw new ClientMetadataError("client_id must name a host, not an address");
  return u;
}

/** The socket's own resolver: every address it would use must be public, or the connect fails. */
const safeLookup: typeof dnsLookup = ((hostname: string, options: unknown, callback: (err: NodeJS.ErrnoException | null, address: string | LookupAddress[], family?: number) => void) => {
  dnsLookup(hostname, { all: true }, (err, addresses) => {
    if (err) return callback(err, "", 0);
    const list = addresses as LookupAddress[];
    const bad = list.find((a) => !isPublicAddress(a.address));
    if (list.length === 0 || bad) {
      return callback(Object.assign(new Error(`refusing to fetch client metadata from a non-public address (${bad?.address ?? "none"})`), { code: "EACCES" }), "", 0);
    }
...
export const fetchDocument: DocumentFetcher = (url) => new Promise((resolve, reject) => {
  const req = request(url, {
    ...
    res.on("data", (c: Buffer) => {
      size += c.length;
      if (size > MAX_BYTES) { req.destroy(new ClientMetadataError("client metadata document is larger than 64 KiB")); return; }
      chunks.push(c);
    });
    res.on("end", () => resolve({
      status: res.statusCode ?? 0, contentType: String(res.headers["content-type"] ?? ""),
      cacheControl: String(res.headers["cache-control"] ?? ""), body: Buffer.concat(chunks).toString("utf8"),
    }));
    res.on("error", reject);
  });
  req.on("timeout", () => req.destroy(new ClientMetadataError("client metadata document took longer than 5 s")));
  req.on("error", reject);
  req.end();
});
...
export function parseDocument(url: string, f: Fetched): ClientMetadata {
  if (f.status !== 200) throw new ClientMetadataError(`client metadata document answered ${f.status}`);
  if (!/^application\/(?:[\w.+-]*\+)?json\b/i.test(f.contentType)) throw new ClientMetadataError("client metadata document is not JSON");
  let doc: unknown;
  try { doc = JSON.parse(f.body); } catch { throw new ClientMetadataError("client metadata document is not valid JSON"); }
  if (!doc || typeof doc !== "object") throw new ClientMetadataError("client metadata document is not an object");
  const d = doc as Record<string, unknown>;
  if (d["client_id"] !== url) throw new ClientMetadataError("client metadata document's client_id does not match its URL");
  const uris = d["redirect_uris"];
  if (!Array.isArray(uris) || uris.length === 0 || uris.length > 20 || !uris.every((u) => typeof u === "string" && u.length <= 2048)) {
  ...
  if (method !== undefined && method !== "none") throw new ClientMetadataError(`token_endpoint_auth_method ${JSON.stringify(method)} is not supported; gangway serves public clients only`);
  ...
  const clientName = rawName.replace(/[\u0000-\u001f\u007f-\u009f\u200b-\u200f\u202a-\u202e\u2066-\u2069]/g, "").slice(0, 80) || new URL(url).hostname;
...
export class ClientMetadataStore {
  ...
  async get(clientId: string): Promise<ClientMetadata> {
    ...
    return this.#flight.run(clientId, async () => {
      let fetched: Fetched;
      try { fetched = await this.#fetch(url); } catch (err) {
        throw err instanceof ClientMetadataError ? err : new ClientMetadataError(`could not fetch the client metadata document: ${(err as Error).message}`);
      }
...
export function redirectAllowed(requested: string, registered: readonly string[]): boolean {
  ...
  return registered.some((raw) => {
    try {
      const g = new URL(raw);
      return loopback(g) && g.hostname === r.hostname && g.pathname === r.pathname && g.search === r.search;
    } catch { return false; }
  });
}
