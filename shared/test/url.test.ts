...
describe("publicUrlFor", () => {
  test("elides the default port for https", () => {
    expect(publicOriginFor("x.preview.example.com", { scheme: "https", port: 443 }))
      .toBe("https://x.preview.example.com");
  });
  test("elides the default port for http", () => {
    expect(publicOriginFor("x.example.com", { scheme: "http", port: 80 }))
      .toBe("http://x.example.com");
  });
  test("keeps a non-default port", () => {
    expect(publicOriginFor("x.preview.localhost", { scheme: "https", port: 8443 }))
      .toBe("https://x.preview.localhost:8443");
  });
  ...
  test("normalises a path without a leading slash", () => {
    expect(publicUrlFor("h", { scheme: "https", port: 443 }, "setup/abc"))
      .toBe("https://h/setup/abc");
  });
