...
): AppPlan => planApp({ paths: Object.keys(files), files, ...extra });
...
describe("node: Heroku's conventions", () => {
  test("npm start by default; install by lockfile; build if there is one", () => {
    const p = plan({
      "package.json": pkg({ scripts: { start: "node server.js", build: "tsc" } }),
      "package-lock.json": "{}",
    });
  ...
  test("pnpm and yarn by their lockfiles", () => {
    expect(
      plan({ "package.json": pkg({ scripts: { start: "x" } }), "pnpm-lock.yaml": "" }).install,
    ).toContain("pnpm install --frozen-lockfile");
    expect(
      plan({ "package.json": pkg({ scripts: { start: "x", build: "y" } }), "yarn.lock": "" }).build,
    ).toBe("yarn run build");
  });
  ...
  test("main, then an entry file, when there is no start script", () => {
    expect(start(plan({ "package.json": pkg({ main: "srv/app.js" }), "srv/app.js": "" }))).toBe(
      "node srv/app.js",
    );
  ...
  test("invalid package.json is an error, not a guess", () => {
    expect(planError(plan({ "package.json": "{ nope" }))).toContain(
...
describe("other runtimes", () => {
  test("python: requirements or pyproject; an entry; Django's manage.py with a warning", () => {
    expect(plan({ "pyproject.toml": "", "main.py": "" }).install).toContain(
  ...
  test("deno: tasks.start beats the entry", () => {
    const p = plan({
      "deno.json": pkg({ tasks: { start: "deno run -A server.ts" } }),
      "server.ts": "",
    });
  ...
  test("bun: a start script, else the Workers-style entry", () => {
    expect(
      start(
        plan({ "bun.lock": "", "package.json": pkg({ scripts: { start: "bun run serve.ts" } }) }),
      ),
    ).toBe("bun run start");
...
describe("where the app is", () => {
  ...
  test("root: in gangway.yml, and a gangway.yml inside the nested app is read", () => {
    expect(
      plan({
        "gangway.yml": "root: apps/site\n",
        "apps/site/index.html": "",
        "apps/api/package.json": "{}",
      }),
    ).toMatchObject({ root: "apps/site", runtime: "static" });
    expect(plan({ "gangway.yml": "root: nope\n" }).issues[0]!.path).toBe("root");
    expect(plan({ "site/gangway.yml": "runtime: static\n", "site/x.txt": "" })).toMatchObject({
      root: "site",
      runtime: "static",
      configFile: "site/gangway.yml",
    });
  });
});
...
describe("a rebuild", () => {
  test("keeps the previous runtime under auto, unless gangway.yml names one", () => {
    ...
    expect(
      plan({ "main.ts": "", "gangway.yml": "runtime: bun\n" }, { previous: "deno" }).runtime,
    ).toBe("bun");
