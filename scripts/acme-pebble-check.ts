...
const dns: DnsProvider = {
  async createTxt(name, value) { seen.push(value); await post("/set-txt", { host: `${name}.`, value }); return { recordId: name }; },
  async removeTxt(_id, name) { await post("/clear-txt", { host: `${name}.` }); },
  async waitForPropagation() { return true; },
};

/** What the listener presents RIGHT NOW for this SNI name. */
const presented = (port: number, servername: string) => new Promise<{ issuer: string; sans: string }>((resolve, reject) => {
...
const start = async () => {
  const config = loadConfig({
    GANGWAY_STATE_DIR: stateDir, GANGWAY_LISTEN_ADDRESS: "127.0.0.1", GANGWAY_LISTEN_PORT: String(freePort()), GANGWAY_LISTEN_HTTP_PORT: "",
    GANGWAY_ADMIN_TOKEN: "gw_pebble_check_0123456789abcdef", GANGWAY_TLS_MODE: "acme", GANGWAY_RECONCILE_INTERVAL_MS: "0",
    GANGWAY_BASE_DOMAIN: BASE, GANGWAY_ACME_DIRECTORY_URL: DIRECTORY,
  }, {});
  return boot(config, {
    acme: { dns }, logger: new Logger("info", {}, (l) => lines.push(l)),
    // No Docker in this check, and above all not whatever socket this machine has.
    clients: { for: () => ({ hostId: "local", info: async () => { throw new Error("no docker in the pebble check"); }, listContainers: async () => [], stopContainer: async () => {} }) },
  });
};
...
try {
  ...
  check("boots serving the dev CA while the first order runs", first.issuer.includes("gangway"), first.issuer);
  ...
  check("the listener presents the ACME certificate WITHOUT a restart", swapped.issuer.includes("Pebble"), swapped.issuer);
  check("it covers the wildcard and the apex", swapped.sans.includes(`DNS:*.${BASE}`) && swapped.sans.includes(`DNS:${BASE}`), swapped.sans);
  check("two DIFFERENT TXT values were published for the one order", new Set(seen).size === 2, `${seen.length} values`);
  check("the API surface still answers on the new certificate", (await fetch(`https://127.0.0.1:${port}/healthz`, { headers: { host: `api.${BASE}` }, tls: { rejectUnauthorized: false } } as RequestInit)).status === 200);
  await running.stop();
  ...
  check("after a restart the stored certificate is served immediately", restarted.issuer.includes("Pebble"), restarted.issuer);
  check("...and NO new order is placed", seen.length === orders, `${seen.length - orders} new TXT records`);
  if (process.exitCode) console.error(lines.filter((l) => l.includes("acme") || l.includes("tls")).join("\n"));
  await running.stop();
} catch (e) {
  console.error(e);
