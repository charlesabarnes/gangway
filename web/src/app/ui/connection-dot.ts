...
@Component({
  ...
  template: `
    @if (status() !== 'idle') {
      <span class="inline-flex items-center gap-1.5 text-xs text-neutral-500" role="status" data-testid="connection">
        <span class="size-1.5 rounded-full" [class]="look().dot" [class.animate-pulse]="look().pulse" aria-hidden="true"></span>{{ look().label }}
      </span>
    }
  `,
})
