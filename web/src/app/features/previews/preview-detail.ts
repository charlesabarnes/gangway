    ...
    <section class="mx-auto max-w-5xl px-6 py-10">
      ...
      @if (preview(); as p) {
        <div class="mt-4 flex flex-wrap items-center gap-x-4 gap-y-2">
          ...
          <span
            class="rounded-full border border-neutral-300 px-2 py-0.5 text-xs text-neutral-500 dark:border-neutral-700"
        ...
        <div class="mt-8 grid gap-8 md:grid-cols-3">
            ...
            >
              @for (u of p.urls; track u.url) {
                <li class="flex items-center gap-3 px-4 py-2.5 text-sm" data-testid="url-row">
                  <span class="w-24 shrink-0 truncate text-neutral-500"
                    >{{ u.service }}
                    @if (u.primary) {
                      <span class="ml-1 text-xs text-accent">primary</span>
                    }
                  </span>
                  <a
                    [href]="u.url"
                    ...
                    class="min-w-0 flex-1 truncate font-mono text-xs hover:text-accent"
                    >{{ u.url }}</a
                  >
                  <button
                    type="button"
                ...
                <li class="px-4 py-2.5 text-sm text-neutral-500">No routes.</li>
              }
            </ul>
          </div>
          ...
          <dl class="space-y-3 text-sm" data-testid="facts">
            ...
            @if (p.state === 'destroyed') {
              ...
              <div>
                <dt class="text-neutral-500">Destroyed</dt>
                <dd class="mt-0.5" [title]="p.destroyedAt ?? ''" data-testid="ttl">
              ...
              @if (p.secretLevel) {
                <div>
                  <dt class="text-neutral-500">Secrets</dt>
                  <dd class="mt-0.5" data-testid="secret-level">
              ...
              @if (p.templateId) {
                <div>
                  <dt class="text-neutral-500">Template</dt>
                  <dd class="mt-0.5">
                    <a
              ...
              <div>
                ...
                <dd class="mt-0.5" [title]="p.ttlExpiresAt ?? ''" data-testid="ttl">
            ...
            <div>
              ...
              <dd class="mt-0.5" [title]="p.createdAt">
                {{ p.createdAt | relativeTime: clock.now() }}
              </dd>
            </div>
            <div>
              <dt class="text-neutral-500">Last visited</dt>
              <dd class="mt-0.5" [title]="p.lastSeenAt ?? ''">
                {{ p.lastSeenAt ? (p.lastSeenAt | relativeTime: clock.now()) : 'not yet' }}
              </dd>
            </div>
            <div>
              <dt class="text-neutral-500">Host</dt>
              <dd class="mt-0.5 font-mono text-xs">{{ p.hostId }}</dd>
            </div>
          </dl>
        </div>
        ...
        @if (
          ...
          <div
            class="mt-2 h-[28rem] overflow-hidden rounded-lg border border-neutral-200 dark:border-neutral-800"
            data-testid="databases"
          >
        ...
        @if (canReadLogs()) {
          ...
          <div class="mt-2">
            @if (p.state === 'destroyed') {
              <p
                class="rounded-lg border border-dashed border-neutral-300 px-4 py-6 text-center text-sm text-neutral-500 dark:border-neutral-700"
        ...
        @if (builds().length > 0) {
          <h2 class="mt-10 text-sm font-medium text-neutral-500">
            Builds
            ...
            @for (b of builds(); track b.id) {
              <li class="flex gap-3">
                <span
                  ...
                  >{{ b.state }}</span
                >
                <span class="text-neutral-500">{{ b.service ?? 'all services' }}</span
                ><span class="ml-auto text-neutral-500" [title]="b.startedAt">{{
        ...
        @if (events().length > 0) {
          ...
          <ol class="mt-2 space-y-1 text-sm" data-testid="events">
            @for (e of events(); track e.seq) {
              <li class="flex gap-3">
                <span class="w-28 shrink-0 text-neutral-500" [title]="e.at">{{
                  e.at | relativeTime: clock.now()
                }}</span>
                <span>
                  @if (e.type === 'preview.state') {
                    {{ e.from }} → <span class="font-medium">{{ e.state }}</span>
                  } @else {
                    {{ e.type.replace('preview.', '') }}
                  }
                </span>
                @if (e.error) {
                  <span class="min-w-0 truncate text-red-600 dark:text-red-400" [title]="e.error">{{
                    e.error
                  }}</span>
                }
              </li>
        ...
        <div class="mt-8">
          <app-empty-state heading="No such preview">
            ...
            <a routerLink="/previews" class="text-accent hover:underline">Back to the list</a>.
          </app-empty-state>
        </div>
      } @else {
