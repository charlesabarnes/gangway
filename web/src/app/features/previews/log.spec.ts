...
const line = (n: number, text = `line ${n}`, stream: LogStream = 'stdout'): LogLine => ({ n, at: '2026-09-21T20:00:00.000Z', stream, line: text });
...
describe('LogViewer', () => {
  ...
  let frames: (() => void)[] = [];
  const nextFrame = () => { const run = frames; frames = []; for (const f of run) f(); };
  ...
  const emit = (n: number, text = `line ${n}`, stream: LogStream = 'stdout') => FakeEventSource.last.emit('log', { at: '2026-09-21T20:00:00.000Z', stream, line: text }, String(n));
  /** `<n> <text>`: two spans whose gap is CSS, so they are read separately. */
  const shown = (r: Rendered<unknown>) => r.allByTestId('line').map((e) => Array.from(e.querySelectorAll('span')).map((s) => s.textContent ?? '').join(' '));
  ...
  it('"build log" is a filter over one stream, not a second request', async () => {
    const r = await open();
    FakeEventSource.last.open();
    emit(1, 'deploying gw-x', 'system'); emit(2, '#1 [internal] load build definition', 'build'); emit(3, '#2 DONE', 'build'); emit(4, 'listening on :80', 'stdout'); emit(5, 'warn: x', 'stderr');
    nextFrame(); await r.settle();
    expect(shown(r)).toHaveLength(5);

    (r.byTestId('stream-build') as HTMLElement).click(); await r.settle();
    expect(shown(r)).toEqual(['2 #1 [internal] load build definition', '3 #2 DONE']);
    expect(FakeEventSource.instances).toHaveLength(1);

    (r.byTestId('stream-stderr') as HTMLElement).click(); await r.settle();
  ...
  it('shows the server\'s "earlier lines not shown" line like any other', async () => {
    const r = await open();
    FakeEventSource.last.open();
    emit(4000, '... 4000 earlier lines not shown', 'system'); emit(4001);
    nextFrame(); await r.settle();
  ...
  describe('scrolling', () => {
    ...
    it('follows the bottom while you are AT the bottom', async () => {
      ...
      FakeEventSource.last.open();
      emit(1); nextFrame(); await r.settle(); nextFrame();
    ...
    it('scrolling up to read something is NOT yanked back down by the next line; a pill offers the way back', async () => {
      ...
      log.scrollTop = 1200; log.dispatchEvent(new Event('scroll')); await r.settle();
      expect(r.byTestId('jump')).not.toBeNull();
      emit(2); nextFrame(); await r.settle(); nextFrame();
      expect(log.scrollTop).toBe(1200);

      (r.byTestId('jump') as HTMLElement).click(); await r.settle();
      expect(log.scrollTop).toBe(5000);
      expect(r.byTestId('jump')).toBeNull();
    });
  });
  ...
  it('closes its stream when it goes away, and reopens for a different preview', async () => {
    ...
    r.fixture.componentRef.setInput('previewId', '01XYZ'); await r.settle();
