  ...
  template: `
    <section class="mx-auto max-w-5xl px-6 py-10">
      ...
      @if (!canDeploy()) {
        ...
        <ul class="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4" data-testid="runtimes">
          @for (r of runtimes(); track r.id) {
            ...
            <li class="text-sm text-neutral-500" data-testid="runtimes-loading">
              {{ runtimesError() ?? 'Loading runtimes…' }}
            </li>
          }
        </ul>
          ...
          @if (upload(); as u) {
            ...
            >
              ...
              @for (r of runtimes(); track r.id) {
                ...
                >
                  <app-brand-icon
                    [path]="looks[r.id].path"
                    [color]="looks[r.id].color"
                    [size]="14"
                  />{{ looks[r.id].name }}
                </button>
              }
            ...
            <p class="mt-1 text-sm text-neutral-500">
              ...
              <span class="group relative ml-1 inline-block">
                ...
                >
                  Add a <code class="font-mono">gangway.yml</code> to set the start command, build
                  step, runtime version or databases. A Dockerfile or compose file is used as is.
                </span>
              </span>
            </p>
            <div class="mt-4 flex justify-center gap-3">
              <label
                class="cursor-pointer rounded-md border border-neutral-300 px-3.5 py-2 text-sm font-medium hover:bg-neutral-100 dark:border-neutral-700 dark:hover:bg-neutral-800"
              >
                Choose files<input
                  type="file"
                  multiple
        ...
        @if (addons().length > 0) {
          ...
          >
            ...
            <p class="text-xs text-neutral-500">
              Temporary databases for this preview. They keep their data between saves and are
              removed with the preview.
            </p>
              ...
              @for (a of addons(); track a.id) {
                ...
                >
                  <input
                    type="checkbox"
                    class="sr-only"
                    [checked]="addonChecks().includes(a.id)"
                  ...
                  <span>
                    ...
                    @if (suggestedBecause(a.id); as why) {
                      <span
        ...
        >
          <summary
            class="cursor-pointer text-sm font-medium text-neutral-600 dark:text-neutral-400"
          >
            Options
          </summary>
          <div class="mt-3 grid gap-3 sm:grid-cols-5">
            ...
            <label class="text-xs text-neutral-500 sm:col-span-2"
              >Project<select
                [class]="field"
                (change)="project.set($any($event.target).value)"
                data-testid="project"
              >
            ...
            <label class="text-xs text-neutral-500 sm:col-span-3"
              >Who can open it<select
                [class]="field"
                (change)="who.set($any($event.target).value)"
                data-testid="who"
              >
                ...
                <option value="either">
                  people signed in to gangway, or anyone with the password
                </option>
              </select></label
            >
            @if (who() === 'password' || who() === 'either') {
              <label class="text-xs text-neutral-500 sm:col-span-2"
                >Password<select
                  [class]="field"
                  (change)="passwordSource.set($any($event.target).value)"
...
export class NewPreview {
  ...
  protected readonly namePlaceholder = computed(() =>
    this.upload() ? 'from the upload' : 'automatic, e.g. bun-k3x9',
  );
  ...
  protected summary(p: AppPlan): string {
    ...
    return [
      ...
      cmd(p.install),
      cmd(p.build),
      serve ?? cmd(p.start) ?? (p.entry ? `runs ${p.entry}` : null),
    ]
      .filter((x) => x)
      .join(' · ');
  }
  ...
  async #post(body: Blob, runtime: Detected | 'auto'): Promise<void> {
    ...
    if (src === 'set' && this.passwordValue() === '') {
      this.error.set({
        status: 422,
        title: 'No password',
        detail: 'Type the preview password, or pick another option.',
        requestId: null,
        retryAfter: null,
        issues: [],
      });
    ...
    try {
      const res = await firstValueFrom(
        this.#http
          .post<{ preview: Preview }>(`/v1/previews${query}`, body, {
            headers,
            reportProgress: true,
            observe: 'events',
          })
