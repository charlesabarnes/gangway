...
import { collectFromFiles, detect, finish, isJunk, normalizePath, packStarter, stripCommonRoot, unzip, UploadError, writeTar, type UploadFile } from './pack';
...
function readTar(tar: Uint8Array): { path: string; type: string; size: number; mode: number; text: string }[] {
  const out = [];
  for (let at = 0; at + 512 <= tar.length;) {
    ...
    out.push({ path, type: str(156, 1), size, mode: parseInt(str(100, 8), 8), text: dec.decode(tar.subarray(at + 512, at + 512 + size)) });
...
describe('packing an upload', () => {
  it('writes a ustar archive a reader parses back: regular files only, checksums right', () => {
    const tar = writeTar([file('index.html', '<h1>hi</h1>'), file('assets/app.js', 'x'.repeat(700))], 1_700_000_000);
    expect(tar.length % 512).toBe(0);
    const entries = readTar(tar);
    expect(entries.map((e) => [e.path, e.type])).toEqual([['index.html', '0'], ['assets/app.js', '0']]);
  ...
  it('a starter becomes a gzipped tar of exactly its files', () => {
    const blob = packStarter({ 'src/index.ts': 'export default {};\n', 'wrangler.toml': 'main = "src/index.ts"\n' });
  ...
  it('skips OS litter, VCS and dependencies', () => {
    for (const p of ['__MACOSX/x', '.git/config', 'a/node_modules/b.js', '.DS_Store', 'd/Thumbs.db', 'img/._photo.png']) expect(isJunk(p)).toBe(true);
    for (const p of ['index.html', 'src/git.ts', 'modules/x.js', '.env.example']) expect(isJunk(p)).toBe(false);
  });
  ...
  it('strips ONE common root folder and remembers it as the name', () => {
    expect(stripCommonRoot([file('site/index.html'), file('site/css/a.css')])).toEqual({ root: 'site', files: [file('index.html', 'site/index.html'), file('css/a.css', 'site/css/a.css')] });
  ...
  it('finish(): junk counted, root stripped, sorted, duplicates refused', () => {
    const c = finish([file('app/package.json'), file('app/node_modules/x/index.js'), file('app/.DS_Store'), file('app/server.js')]);
  ...
  it('a zip is expanded in the browser, directories dropped', async () => {
    const zip = zipSync({ 'site/': new Uint8Array(0), 'site/index.html': strToU8('<p>zip</p>'), 'site/a/b.css': strToU8('b{}') });
    expect(unzip(zip).map((f) => f.path).sort()).toEqual(['site/a/b.css', 'site/index.html']);
    ...
    const flat = await collectFromFiles([new File([zipSync({ 'index.php': strToU8('<?php') }) as Uint8Array<ArrayBuffer>], 'blog.zip')]);
