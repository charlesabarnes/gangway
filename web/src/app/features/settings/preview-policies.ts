...
@Component({
  selector: 'app-preview-policies',
  imports: [Btn, ConfirmDialog, DefaultTemplates],
  host: { class: 'block' },
  template: `
    <div class="gw-section" id="policies">
      <div class="flex flex-col gap-1">
        <h2 class="gw-h2">Preview policies</h2>
        <p class="gw-section-note">
          What a preview gets when nothing more specific is said: its visibility, how long it lives,
          when it sleeps, which secrets it may read, and where it runs. A repository can pick its
          own in its settings.
        </p>
      </div>
      <div class="flex min-w-0 flex-col gap-8">
          ...
          @for (t of templates(); track t.id) {
            ...
            >
              <div class="flex flex-wrap items-baseline gap-x-3 gap-y-1">
                <span class="font-serif text-[22px]">{{ t.name }}</span>
                ...
                >
                  ...
                  <div class="flex flex-wrap items-center gap-3.5 sm:col-span-6">
                    <button
                      ...
                      size="sm"
                      type="submit"
                      [disabled]="!dirty(t) || savingRow() === t.id"
                      data-testid="save"
                    >
                      Save
                    </button>
        ...
        @if (canManage()) {
          <div class="flex flex-col gap-3">
            <div class="flex flex-col gap-1">
              <h3 class="gw-label">New policy</h3>
      ...
      >
        Repositories on it go back to the default for pull requests. Running previews keep what they
        were deployed with.
      </app-confirm-dialog>
    </div>
  `,
})
export class PreviewPolicies {
  ...
  protected async save(e: Event, t: Template): Promise<void> {
    ...
    if (!patch || !this.dirty(t) || this.savingRow()) return;
