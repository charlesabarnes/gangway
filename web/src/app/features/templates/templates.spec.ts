...
const inRow = (r: Rendered<unknown>, n: number, id: string) => r.allByTestId('template')[n]!.querySelector(`[data-testid="${id}"]`) as HTMLElement;
...
describe('Templates', () => {
  ...
  it('edits are a draft per row and saved as a PATCH of only what changed; the built-in one is editable but has no delete', async () => {
    ...
    const idle = inRow(r, 0, 'idle') as HTMLInputElement; idle.value = 'never'; idle.dispatchEvent(new Event('input'));
    await choose(r, 'clearance', 'high');
    // The SECOND row's name: must not leak into the first row's patch.
    const other = inRow(r, 1, 'name') as HTMLInputElement; other.value = 'Other'; other.dispatchEvent(new Event('input'));
    await r.settle();
    expect(save().disabled).toBe(false);
    r.byTestId('form-default')!.dispatchEvent(new Event('submit', { cancelable: true })); await r.settle();
  ...
  it('a 422 (a bad duration) is shown on that row', async () => {
    const r = await open();
    type(r, 'ttl', 'soon'); await r.settle();
    r.byTestId('form-default')!.dispatchEvent(new Event('submit', { cancelable: true })); await r.settle();
    r.http.expectOne({ method: 'PATCH', url: '/v1/templates/default' }).flush({ type: 'about:blank', title: 'Unprocessable', status: 422, detail: 'ttl "soon" is not a duration like 12h or 7d' }, { status: 422, statusText: 'Unprocessable' });
  ...
  it('create: an id that is a slug and a name; the new template joins the list', async () => {
    ...
    type(r, 'new-id', 'Not A Slug'); type(r, 'new-name', 'X'); await r.settle();
    expect(btn().disabled).toBe(true);
    type(r, 'new-id', 'ci'); type(r, 'new-name', 'CI'); await r.settle();
    ...
    req.flush({ template: template({ id: 'ci', name: 'CI', builtin: false }) }, { status: 201, statusText: 'Created' });
    await r.settle();
  ...
  it('a taken id is said next to the form', async () => {
    const r = await open();
    type(r, 'new-id', 'staging'); type(r, 'new-name', 'Again'); await r.settle();
    r.byTestId('create')!.dispatchEvent(new Event('submit', { cancelable: true })); await r.settle();
    r.http.expectOne({ method: 'POST', url: '/v1/templates' }).flush({ type: 'about:blank', title: 'Conflict', status: 409, detail: 'template "staging" already exists' }, { status: 409, statusText: 'Conflict' });
  ...
  it('delete asks first; confirmed, it is a DELETE and the row goes; refused (a trigger default), it stays with a toast', async () => {
    const r = await open();
    (inRow(r, 1, 'delete') as HTMLButtonElement).click(); await r.settle();
    ...
    r.http.expectOne({ method: 'DELETE', url: '/v1/templates/staging' }).flush({ type: 'about:blank', title: 'Conflict', status: 409, detail: '"staging" is the default template for api; point those elsewhere first' }, { status: 409, statusText: 'Conflict' });
    await r.until(() => (r.el.textContent ?? '').includes('Could not delete Staging'), 'toast');
    expect(r.allByTestId('template')).toHaveLength(2);

    (inRow(r, 1, 'delete') as HTMLButtonElement).click(); await r.settle();
    dialog.close('confirm'); await r.settle();
    r.http.expectOne({ method: 'DELETE', url: '/v1/templates/staging' }).flush(null, { status: 204, statusText: 'No Content' });
