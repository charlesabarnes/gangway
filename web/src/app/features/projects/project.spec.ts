...
import { PERMISSIONS, type Permission, type Preview, type Project, type Template } from '../../core/api.types';
...
@Component({ imports: [ProjectPage, Toasts], template: '<app-project ref="web-app" /><app-toasts />' })
class Host {}
...
async function open(o: { permissions?: Permission[]; project?: Project; previews?: Preview[]; tab?: string } = {}) {
  FakeEventSource.reset();
  const perms = o.permissions ?? [...PERMISSIONS];
  const r = await render(Host, {
    routes: [{ path: '**', component: Blank }],
    providers: [{ provide: EVENT_SOURCE_FACTORY, useValue: (url: string) => new FakeEventSource(url) }, { provide: SSE_JITTER, useValue: () => 0 }],
  });
  ...
  r.http.expectOne('/v1/templates').flush({ templates: [template(), template({ id: 'ci', name: 'CI', builtin: false })] });
  r.http.expectOne((req) => req.url.startsWith('/v1/previews')).flush({ seq: 1, previews: o.previews ?? [] });
  await r.settle();
  return r;
}
...
describe('Project', () => {
  ...
  it('workflow tab: the file for this project, re-fetched when the port changes', async () => {
    ...
    const port = r.byTestId('port') as HTMLInputElement; port.value = '8080'; port.dispatchEvent(new Event('change')); await r.settle();
  ...
  it('settings: a PATCH of only what changed; "the template\'s" is null; a changed slug moves the URL', async () => {
    ...
    const slug = r.byTestId('slug') as HTMLInputElement; slug.value = 'store'; slug.dispatchEvent(new Event('input')); await r.settle();
    await choose(r, 'visibility', ''); // back to the template's: equal to the saved null, so not sent
    r.byTestId('settings')!.dispatchEvent(new Event('submit', { cancelable: true })); await r.settle();
    ...
    await r.until(() => TestBed.inject(Router).url === '/projects/store?tab=settings', 'navigation');
  });

  it('delete asks first, then leaves for the list', async () => {
    const r = await open({ tab: 'settings' });
    (r.byTestId('delete') as HTMLButtonElement).click(); await r.settle();
    const dialog = r.byTestId('confirm') as HTMLDialogElement;
    expect(dialog.open).toBe(true);
    dialog.close('confirm'); await r.settle();
    r.http.expectOne({ method: 'DELETE', url: `/v1/projects/${P.id}` }).flush(null, { status: 204, statusText: 'No Content' });
  ...
  it('secrets tab talks to the project\'s env', async () => {
    ...
    r.http.expectOne(`/v1/projects/${P.id}/env`).flush({ secrets: [{ name: 'API_KEY', level: 'standard' }] });
    await r.settle();
  ...
  it('an unknown project says so', async () => {
    FakeEventSource.reset();
    const r = await render(Host, { routes: [{ path: '**', component: Blank }], providers: [{ provide: EVENT_SOURCE_FACTORY, useValue: (url: string) => new FakeEventSource(url) }, { provide: SSE_JITTER, useValue: () => 0 }] });
    r.http.expectOne('/v1/projects/web-app').flush({ type: 'about:blank', title: 'Not Found', status: 404, detail: 'no such project: web-app' }, { status: 404, statusText: 'Not Found' });
