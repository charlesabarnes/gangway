...
const project = (over: Partial<Project> = {}): Project => ({ ...(contract.project as Project), ...over });
const template = (over: Partial<Template> = {}): Template => ({ ...(contract.template as Template), ...over });
const preview = (over: Partial<Preview> = {}): Preview => ({ ...(contract.preview as Preview), ...over });

async function open(o: { permissions?: Permission[]; projects?: Project[]; previews?: Preview[]; installed?: { fullName: string; installationId: string; private: boolean }[] } = {}) {
  ...
  if (perms.includes('repos.manage')) r.http.expectOne('/v1/github/repositories').flush({ repositories: o.installed ?? [] });
...
describe('Projects', () => {
  it('each project as a card: its repository and trigger, its template, its live previews; previews in no project listed below', async () => {
    ...
    expect(r.allByTestId('source').map((e) => e.textContent?.trim())).toEqual(['acme/web-app · workflow', 'no repository']);
  ...
  it('a project with no repository sends no trigger; a refusal is shown in the form', async () => {
    const r = await open();
    (r.byTestId('new') as HTMLButtonElement).click(); await r.settle();
    type(r, 'create-name', 'whoami'); await r.settle();
    r.byTestId('create')!.dispatchEvent(new Event('submit', { cancelable: true })); await r.settle();
    const req = r.http.expectOne({ method: 'POST', url: '/v1/projects' });
    expect(req.request.body).toEqual({ name: 'whoami' });
    req.flush({ type: 'about:blank', title: 'Conflict', status: 409, detail: 'slug "whoami" is taken' }, { status: 409, statusText: 'Conflict' });
