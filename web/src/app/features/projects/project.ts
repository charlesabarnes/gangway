        ...
        @switch (tab()) {
          ...
          @case ('settings') {
            @if (canManage()) {
              ...
              >
                ...
                <section>
                  ...
                  @if (d().repository && d().prTrigger === 'webhook') {
                    ...
                    <p class="mt-2 text-xs text-neutral-500">
                      Pull requests from forks are skipped: GitHub gives their workflow runs no OIDC
                      token.
                    </p>
                  }
                </section>
                    ...
                    <label class="text-xs text-neutral-500"
                      ...
                      >
                        @for (c of prClearances; track c.value) {
                          <option
                            [value]="c.value"
                            [selected]="c.value === (d().prClearance ?? '')"
                          >
                            {{ c.label }}
                          </option>
                        }
                ...
                <div
                  class="flex items-center gap-3 border-t border-neutral-200 pt-4 dark:border-neutral-800"
                >
              ...
              <p class="mt-6 text-sm text-neutral-500">
                Changing a project needs the repos.manage permission.
              </p>
            }
          }
          @case ('secrets') {
            <div class="mt-6" data-testid="secrets">
              <p class="text-sm text-neutral-600 dark:text-neutral-400">
                Given to this project's previews at or below their clearance, on top of the global
                ones in Settings; a name here wins.
                @if (p.prTrigger === 'workflow' && p.fullName) {
                  With a workflow they reach the running container as its environment — build-time
          ...
          @case ('workflow') {
            <div class="mt-6 space-y-4" data-testid="workflow">
              <ol
                class="list-decimal space-y-1.5 pl-5 text-sm text-neutral-700 dark:text-neutral-300"
              >
                <li>
                ...
                <li>
                  Commit this file to <span class="font-mono">{{ p.fullName }}</span> as
                  <code class="font-mono">.github/workflows/gangway-preview.yml</code>. The image is
                  built from the repository's Dockerfile.
                </li>
                <li>
                  Open a pull request. The run builds, pushes to
                  <span class="font-mono">ghcr.io</span>, and gangway runs it; a comment on the PR
                  has the URL.
                </li>
              </ol>
              <div class="relative">
...
export class ProjectPage {
  ...
  constructor() {
    ...
    effect(() => {
      if (this.tab() === 'secrets' && this.project()) untracked(() => void this.#loadSecrets());
    });
    effect(() => {
      if (this.tab() === 'workflow' && this.project()) {
        this.port();
        untracked(() => void this.#loadYaml());
      }
    });
  }
      ...
      this.secrets.set(
        (
          await firstValueFrom(
            this.#http.get<{ secrets: SecretListing[] }>(`/v1/projects/${this.project()!.id}/env`),
          )
        ).secrets,
      );
  ...
  async #loadYaml(): Promise<void> {
    try {
      this.yaml.set(
        await firstValueFrom(
          this.#http.get(`/v1/projects/${this.project()!.id}/workflow?port=${this.port()}`, {
            responseType: 'text',
          }),
        ),
      );
  ...
  protected async copy(): Promise<void> {
    try {
      ...
      this.#toasts.info('Copied the workflow');
    } catch {
      this.#toasts.info(
        'Select the text and copy it',
        'The browser would not let the page write to the clipboard.',
      );
    }
  }
  ...
  protected async save(e: Event): Promise<void> {
    ...
    const patch = Object.fromEntries(
      Object.entries(this.draft()).filter(([k, v]) => saved[k] !== v),
    );
    this.saving.set(true);
    this.saveError.set(null);
    try {
      const { project } = await firstValueFrom(
        this.#http.patch<{ project: Project }>(`/v1/projects/${p.id}`, patch),
      );
      this.project.set(project);
      this.draft.set({});
      ...
      this.saveError.set(
        pr.issues.length ? pr.issues.map((i) => `${i.path}: ${i.message}`).join('; ') : pr.detail,
      );
    } finally {
