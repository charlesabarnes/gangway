...
async function open(caps: Capabilities = { ...(contract.capabilities as Capabilities), surfaces: { ui: true, mcp: true }, mcpUrl: MCP }) {
...
describe('ConnectAgent', () => {
  it('fills THIS server\'s MCP URL into every client\'s recipe, Claude Code first', async () => {
    const r = await open();
    const codes = () => Array.from(r.el.querySelectorAll('[data-testid="agent-code"]')).map((e) => e.textContent ?? '');
    expect(codes()[0]).toBe(`claude plugin marketplace add ${PLUGIN_REPO} && claude plugin install gangway@gangway --config mcp_url=${MCP}/`);
    ...
    expect(JSON.parse(decodeURIComponent(link.getAttribute('href')!.replace('vscode:mcp/install?', '')))).toEqual({ name: 'gangway', type: 'http', url: `${MCP}/` });
  });
