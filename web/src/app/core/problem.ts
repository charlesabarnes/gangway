...
export function toProblem(e: unknown): ProblemError {
  const base: ProblemError = { status: 0, title: 'Something went wrong', detail: 'An unexpected error occurred.', requestId: null, retryAfter: null, issues: [] };
  if (!(e instanceof HttpErrorResponse)) return { ...base, detail: e instanceof Error ? e.message : base.detail };

  // status 0: the request never got an answer -- offline, DNS, or the server is gone.
  if (e.status === 0) return { ...base, title: 'Cannot reach the server', detail: 'The request did not get a response. Check your connection; gangway may be restarting.' };

  const body: Record<string, unknown> = e.error !== null && typeof e.error === 'object' ? (e.error as Record<string, unknown>) : {};
  ...
  const issues = Array.isArray(body['issues'])
    ? (body['issues'] as unknown[]).flatMap((i) => (i !== null && typeof i === 'object' ? [{ path: String((i as Record<string, unknown>)['path'] ?? ''), message: String((i as Record<string, unknown>)['message'] ?? '') }] : []))
    : [];

  // 502/503/504 with no problem body is a reverse proxy talking, not gangway.
  const restarting = (e.status === 502 || e.status === 503 || e.status === 504) && str(body['detail']) === null;
  return {
    status: e.status,
    title: restarting ? 'gangway is restarting' : (str(body['title']) ?? `HTTP ${e.status}`),
    detail: restarting ? 'The server is not answering yet. This usually clears in a few seconds.' : (str(body['detail']) ?? e.statusText ?? base.detail),
