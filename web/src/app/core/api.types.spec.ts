...
describe('the /v1 wire contract', () => {
  it('the fixture satisfies the hand-written types', () => {
    ...
    const PREVIEW_KEYS: (keyof Preview)[] = ['id', 'project', 'hostId', 'kind', 'state', 'source', 'visibility', 'ttlExpiresAt', 'idleAfterMs', 'secretLevel', 'templateId', 'projectId', 'password', 'passwordLogin', 'lastSeenAt', 'error', 'createdAt', 'updatedAt', 'destroyedAt', 'urls'];
