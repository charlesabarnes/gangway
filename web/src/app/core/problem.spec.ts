...
describe('toProblem', () => {
    ...
    const p = toProblem(
      new HttpErrorResponse({
        ...
        error: {
          ...
          requestId: '01ABC',
          issues: [{ path: 'password', message: 'at least 12 characters' }],
        },
      }),
    );
    expect(p).toEqual({
  ...
  it("a proxy's bare 502 is a restart, while gangway's own 503 keeps its words", () => {
    ...
    const draining = toProblem(
      new HttpErrorResponse({
        status: 503,
        error: { title: 'unavailable', detail: 'gangway is shutting down', requestId: '01X' },
      }),
    );
    expect(draining).toMatchObject({
      title: 'unavailable',
      detail: 'gangway is shutting down',
      requestId: '01X',
    });
  });
